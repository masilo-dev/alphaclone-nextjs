-- Re-apply the corrected convert_lead_to_contact definition in a new migration.
-- The original 20260625210000 migration may already be recorded as applied in production,
-- so editing that historical file alone would not update deployed databases.

-- Align convert_lead_to_contact RPC parameter names with the Supabase client (lead_id, not p_lead_id)
-- and return jsonb { contact_id, client_id, company_id } for the app.
...
GRANT EXECUTE ON FUNCTION public.convert_lead_to_contact(uuid, boolean, text, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.convert_lead_to_contact(uuid, boolean, text, text) TO service_role;
