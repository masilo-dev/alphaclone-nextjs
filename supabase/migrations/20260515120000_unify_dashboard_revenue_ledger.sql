-- Migration: Unify Dashboard Revenue with General Ledger
-- Date: 2026-05-15
-- Description: Updates the dashboard stats RPC to pull Total Revenue from the accounting ledger.
...
BEGIN
    ...
    IF to_regclass('public.journal_entry_lines') IS NOT NULL AND to_regclass('public.chart_of_accounts') IS NOT NULL THEN
        ...
        -- Pending Revenue still comes from Invoices (Operational data)
    ...
    ELSIF to_regclass('public.business_invoices') IS NOT NULL THEN
        -- Fallback to invoices if accounting module tables are missing
        SELECT 
    ...
    IF to_regclass('public.audit_logs') IS NOT NULL THEN
        ...
        IF p_user_id IS NOT NULL THEN
            ...
            streaks AS (
                SELECT 
                    activity_day,
                    activity_day - ROW_NUMBER() OVER (ORDER BY activity_day DESC)::integer AS group_id
                FROM daily_activity
            )
