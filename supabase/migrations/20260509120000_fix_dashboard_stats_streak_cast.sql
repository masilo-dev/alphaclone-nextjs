-- Fix: operator does not exist: date - bigint in get_consolidated_dashboard_stats
-- The streak calculation uses: activity_day - ROW_NUMBER() OVER (...)
-- ROW_NUMBER() returns bigint, but PostgreSQL requires integer for date arithmetic.
-- Fix: explicitly cast ROW_NUMBER() result to integer.
-- Date: 2026-05-09
    ...
    IF to_regclass('public.audit_logs') IS NOT NULL THEN
        ...
        IF p_user_id IS NOT NULL THEN
            ...
            streaks AS (
                SELECT 
                    activity_day,
                    -- FIX: cast ROW_NUMBER() bigint → integer for date arithmetic
