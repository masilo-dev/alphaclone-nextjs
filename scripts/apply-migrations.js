...
if (!DATABASE_URL) {
  ...
  console.error(
    '  2. Pass as environment variable: DATABASE_URL="postgresql://..." npm run migrate',
  );
  console.error("");
  console.error(
    "Get connection string from: https://supabase.com/dashboard → Settings → Database",
  );
...
async function verifyMigrations(client) {
  ...
  if (foundTables.includes("subscription_tier_limits")) {
    ...
    if (tiers.length > 0) {
      ...
      tiers.forEach((tier) => {
        const storage =
          tier.storage_mb_limit === 999999 ? "∞" : `${tier.storage_mb_limit}MB`;
        console.log(
          `     - ${tier.tier_name}: ${tier.users_limit} users, ${tier.projects_limit} projects, ${storage}`,
        );
      });
    } else {
