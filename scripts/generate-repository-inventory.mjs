...
function walk(directory) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (entry.isFile()) results.push(relative(root, absolute).replaceAll('\\', '/'));
  }
  return results;
}

const inventoryRoots = ['src', 'app', 'supabase', 'tests', 'scripts', '.github'];
const rootFiles = [
  'package.json',
  'next.config.ts',
  'proxy.ts',
...
const files = [
  ...inventoryRoots.flatMap((directory) => {
    try {
      return statSync(resolve(root, directory)).isDirectory() ? walk(resolve(root, directory)) : [];
    } catch {
      return [];
    }
  }),
...
const pages = files.filter((file) => /^src\/app\/.+\/page\.[^.]+$/.test(file) || /^src\/app\/page\.[^.]+$/.test(file));
const routeHandlers = files.filter((file) => /^src\/app\/.+\/route\.[^.]+$/.test(file));
const apiRoutes = routeHandlers.filter((file) => file.startsWith('src/app/api/'));
const serverActions = matchingFiles(/^\s*['"]use server['"];?/m);
const clientServices = files.filter((file) => /^src\/services\/.+\.[cm]?[jt]sx?$/.test(file));
const migrations = files.filter((file) => /^(?:src\/)?supabase\/migrations\/.+\.sql$/.test(file));
const canonicalMigrations = migrations.filter((file) => file.startsWith('supabase/migrations/'));
const legacyMigrations = migrations.filter((file) => file.startsWith('src/supabase/migrations/'));
const tests = files.filter((file) => /(?:^|\/)(?:__tests__\/.*|[^/]+\.(?:test|spec)\.[^.]+)$/.test(file));
const cronRoutes = apiRoutes.filter((file) => /\/(?:cron|scheduled|scheduler|worker|queue)\//i.test(file));
const workers = files.filter((file) => /(?:^|\/)(?:worker|workers|jobs|queue)(?:\/|\.)/i.test(file));
const services = files.filter((file) => /^src\/services\/.+\.[cm]?[jt]sx?$/.test(file));
const modals = files.filter((file) => /(?:Modal|Dialog|Drawer)\.(?:tsx|jsx)$/.test(file));
const navigation = matchingFiles(/(?:navigation|sidebar|menuItems|navItems|href\s*:)/i);
const destructiveActions = matchingFiles(/(?:\.delete\s*\(|window\.confirm|\bdelete[A-Z_]|\barchive[A-Z_]|hard.?delete|permanent.?delete)/i);
const publicUrls = uniqueMatches(/\b(https?:\/\/[^\s'"`)<>,]+)/g);
const environmentVariables = uniqueMatches(/(?:process\.env\.|process\.env\[['"])([A-Z][A-Z0-9_]*)/g);
const featureFlags = environmentVariables.filter((name) => /(?:ENABLE|DISABLE|FEATURE|FLAG|EXPERIMENT)/.test(name));
const tables = uniqueMatches(/\b(?:create\s+table(?:\s+if\s+not\s+exists)?|alter\s+table(?:\s+if\s+exists)?)\s+(?:public\.)?["']?([a-zA-Z_][\w]*)/gi);
const storageBuckets = uniqueMatches(/(?:storage\.buckets[\s\S]{0,240}?values\s*\(\s*['"]|bucket\s*[:=]\s*['"])([a-zA-Z0-9_-]+)/gi);
const externalIntegrations = matchingFiles(/(?:stripe|zoho|calendly|google|microsoft|linkedin|twilio|whatsapp|sendgrid|resend|brevo|slack|zoom|hubspot|deepseek|openai|anthropic)/i);
const mcpFiles = files.filter((file) => /(?:^|\/)(?:mcp|bonnie|nexus)(?:\/|[A-Z_.-])/i.test(file));
...
const findings = Object.fromEntries([
  ...
  ['raw SQL', /(?:\.rpc\s*\(\s*['"]exec|\bexecute\s+immediate\b|\braw\s*sql\b)/i],
  ['direct browser writes', /createBrowserClient[\s\S]{0,1200}?\.(?:insert|update|delete)\s*\(/i],
...
const matrix = Object.entries(moduleDefinitions).map(([module, keywords]) => {
  ...
  const permissionFiles = textFiles.filter((file) => includesKeyword(file, keywords) && /(?:permission|authorize|membership|tenant)/i.test(read(file)));
  const moduleTests = tests.filter((file) => includesKeyword(file, keywords));
  let status = 'Broken';
  if (ui.length && api.length && database.length && permissionFiles.length && moduleTests.length) status = 'Partially complete';
  else if (ui.length && !api.length && !database.length) status = 'UI only';
  else if (!ui.length && (api.length || database.length)) status = 'Backend only';
  else if (ui.length || api.length || database.length) status = 'Partially complete';
  return { module, ui, api, database, permissions: permissionFiles, tests: moduleTests, status };
});
...
const lines = [
  ...
  '| --- | ---: | ---: | ---: | ---: | ---: | --- |',
  ...matrix.map((entry) => `| ${entry.module} | ${entry.ui.length} | ${entry.api.length} | ${entry.database.length} | ${entry.permissions.length} | ${entry.tests.length} | ${entry.status} |`),
  '',
  '## Risk-marker scan',
  '',
  '| Marker | Files |',
  '| --- | ---: |',
  ...Object.entries(findings).map(([name, matches]) => `| ${name} | ${matches.length} |`),
  '',
  '## Detailed artifacts',
  '',
];
...
for (const [heading, values] of [
  ...
  lines.push(`### ${heading}`, '', ...(values.length ? values.map((value) => `- \`${value}\``) : ['- None found']), '');
}

lines.push('## Reproduction', '', '```bash', 'node scripts/generate-repository-inventory.mjs', '```', '');
writeFileSync(resolve(root, 'docs/AUDIT_REPOSITORY_INVENTORY.md'), `${lines.join('\n')}\n`);

console.log(`Inventory written to ${relative(root, jsonPath)} (${files.length} files scanned).`);
