...
function loadEnv() {
    ...
    if (!fs.existsSync(envPath)) {
        ...
        console.log('Please ensure .env.local exists with VITE_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY');
        process.exit(1);
    }
...
async function main() {
    ...
    for (const client of CLAUDE_CLIENTS) {
        try {
            ...
            if (error) {
                console.error(`❌ Failed to register ${client.client_id}:`, error.message);
                errorCount++;
            } else {
