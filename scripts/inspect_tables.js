...
async function inspect() {
  ...
  for (const target of targets) {
    ...
    if (error) {
      ...
      const { data: cols, error: colErr } = await supabase.rpc('inspect_columns_fallback', { tbl: target }).catch(() => ({ error: true }));
      if (colErr) {
        // Try selecting a non-existent column to see error message which might list columns, or do another check
        const { error: dummyErr } = await supabase.from(target).select('non_existent_column_dummy').limit(1);
        if (dummyErr) {
          console.log(`Postgrest error (helps identify schema):`, dummyErr.message);
        }
      }
    }
  }
