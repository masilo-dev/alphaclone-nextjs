...
function detectRequestParam(src) {
    const m = src.match(/export\s+async\s+function\s+(?:GET|POST|PUT|PATCH|DELETE|HEAD|OPTIONS)\s*\(\s*(\w+)/);
    return m?.[1] ?? 'req';
}

function ensureImport(src) {
    if (src.includes("from '@/lib/api/clientErrorResponse'")) return src;
    const line = "import { clientErrorResponse } from '@/lib/api/clientErrorResponse';\n";
...
function scopeFromFile(file) {
    return path.relative(apiRoot, file).replace(/\\/g, '/').replace(/\/route\.ts$/, '');
}
...
function transformFile(file) {
    ...
    const orig = s;
    if (!/\berror\.message\b|\berr\.message\b|\bdetails:\s*(?:error|err|e)\.message\b/.test(s)) {
        return false;
    }
