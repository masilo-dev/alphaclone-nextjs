...
async function checkTenants() {
  ...
  const { data: tu, error: tuErr } = await supabase.from('tenant_users').select('*').limit(10);
  if (tuErr) console.error(tuErr);
  else console.log(tu);

  console.log('--- business_invoices ---');
  const { data: inv, error: invErr } = await supabase.from('business_invoices').select('id, tenant_id, invoice_number, total, status').limit(5);
