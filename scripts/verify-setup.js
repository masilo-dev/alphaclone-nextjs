...
const checks = {
  ...
  recommended: [
    ...
    },
    {
      ...
      description:
        "For webhooks. Configure at: https://dashboard.stripe.com/webhooks",
      category: "Payments",
    },
  ],
  optional: [
...
checks.required.forEach((check) => {
  const value =
    process.env[check.key] || (check.alt ? process.env[check.alt] : null);
  ...
  if (isValid) {
    ...
    console.log(`❌ ${check.name}`);
    console.log(
      `   Variable: ${check.key}${check.alt ? ` or ${check.alt}` : ""}`,
    );
...
console.log(
  `   Recommended: ${recommendedCount}/${recommendedTotal} (${Math.round((recommendedCount / recommendedTotal) * 100)}%)`,
);
console.log(
  `   Optional: ${optionalCount}/${optionalTotal} (${Math.round((optionalCount / optionalTotal) * 100)}%)`,
);
