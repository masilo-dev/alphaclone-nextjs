...
test.describe("Unified Inbox", () => {
  ...
  test("Mail route loads merged inbox with mailbox and channels tabs", async ({
    page,
  }) => {
  ...
  test("Channels tab loads aggregated feed", async ({ page }) => {
    ...
    await expect(
      page.getByText(/Unified Inbox|All Channels|Pending/i).first(),
    ).toBeVisible({
      timeout: 15000,
    });
  });
  ...
  test("Accessibility: inbox tabs and search are labeled", async ({ page }) => {
    await page.goto("/dashboard/mail", { timeout: 60000 });
    await expect(
      page.getByRole("tablist", { name: "Inbox views" }),
