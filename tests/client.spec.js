...
test.describe("Client Dashboard Audit", () => {
  ...
  test("Navigation: Core pages load", async ({ page }) => {
    ...
    for (const p of pages) {
      ...
      await expect(page.locator("text=Establishing Neural Link")).toHaveCount(
        0,
        { timeout: 20000 },
      );
      ...
      await expect(page.locator("header, nav, #main-content")).not.toHaveCount(
        0,
        { timeout: 5000 },
      );
    }
  ...
  test("Core Flow: My Projects Details", async ({ page }) => {
    ...
    const projectItem = page
      .locator('.project-card, tr.project-row, [data-testid="project-item"]')
      .first();
  ...
  test("Core Flow: Submit Request", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Project Details, text=Submit New Project"),
    ).not.toHaveCount(0);

    // Fill form if fields are visible
    const subject = page.locator(
      'input[name="subject"], input[placeholder*="Name"]',
    );
