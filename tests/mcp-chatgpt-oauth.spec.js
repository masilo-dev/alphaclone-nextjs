...
test.describe('MCP ChatGPT OAuth', () => {
  test('Authorize page requires login for ChatGPT connector', async ({ page }) => {
  ...
  test('OAuth approve rejects unauthenticated requests', async ({ request }) => {
  ...
  test('MCP well-known discovery is public', async ({ request }) => {
    const res = await request.get('/api/mcp/well-known/oauth-authorization-server');
