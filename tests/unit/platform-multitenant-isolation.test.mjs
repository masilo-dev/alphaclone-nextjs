...
test('tenantCacheKey and tenantStoragePath require tenant prefix', () => {
  ...
  assert.equal(tenantCacheKey(tid, 'crm', 'contacts'), `tenant:${tid}:crm:contacts`);
  assert.match(
...
test('assertCronRowTenantContext quarantines missing tenant_id', () => {
  assert.throws(
    ...
    (err) => err instanceof PlatformTenantError && /quarantined/i.test(err.message)
  );
  assert.equal(
    assertCronRowTenantContext({
      id: 'x',
...
test('MCPServer requireTenant no longer trusts args.tenant_id (source)', async () => {
  ...
  assert.equal(/args\.tenant_id[\s\S]{0,80}must be a valid workspace UUID/.test(src), false);
});
...
test('defineConnectorTool requires session tenant (source)', async () => {
  ...
  assert.equal(src.includes('context.tenantId || (args as any).tenant_id'), false);
});
