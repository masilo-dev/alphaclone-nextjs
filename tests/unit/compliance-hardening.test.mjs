...
test("password policy requires 12 chars", async () => {
  const { validatePasswordPolicy } =
    await import("../../src/lib/security/passwordPolicy.ts");
...
test("contract audit trail PDF generator exists", () => {
  const src = fs.readFileSync(
    new URL(
      "../../src/lib/contracts/generateContractAuditTrailPdf.ts",
      import.meta.url,
    ),
    "utf8",
  );
...
test("AI policy and risk register docs exist", () => {
  assert.ok(
    fs.existsSync(
      new URL("../../docs/AI_POLICY_STATEMENT.md", import.meta.url),
    ),
  );
  assert.ok(
    fs.existsSync(new URL("../../docs/AI_RISK_REGISTER.md", import.meta.url)),
  );
});
