...
const { isRedirectUriAllowed, CHATGPT_OAUTH_REDIRECT_URIS, normalizeMcpResourceUrl } = await import(
  '../../src/lib/mcp/oauthRedirect.ts'
);

const { OAUTH_CALLBACKS, listOAuthCallbackUrls } = await import('../../src/lib/config/oauth-callbacks.ts');
const { hasRequiredScopes, MCP_SCOPES, formatScopeString } = await import('../../src/lib/mcp/scopes.ts');
const { createProtectedResourceResponse, createAuthorizationServerResponse, getMcpPublicBaseUrl } =
...
describe('public origin + MCP resource', () => {
  ...
  it('matches token resource against configured resource even when request is 0.0.0.0:8080', () => {
    ...
    assert.notEqual(normalizeResourceUrl(internalRequest), normalizeResourceUrl(PUBLIC_MCP_RESOURCE));
  });

  it('aliases /api/mcp/sse to /api/mcp', () => {
    const sse = 'https://alphaclonesystems.com/api/mcp/sse';
    assert.equal(normalizeMcpResourceUrl(sse), 'https://alphaclonesystems.com/api/mcp');
  ...
  it('buildPublicCallbackUrl rejects non-absolute paths and uses public origin', () => {
    ...
    assert.throws(() => buildPublicCallbackUrl('api/auth/x'), /must start with/);
  });
  ...
  it('OAUTH_CALLBACKS never contain localhost or 0.0.0.0', () => {
    ...
    assert.equal(OAUTH_CALLBACKS.x, 'https://alphaclonesystems.com/api/auth/callback/x');
  });
});
...
describe('redirect URI matching', () => {
  ...
  it('allows path-prefix wildcard under same host', () => {
    assert.equal(
      isRedirectUriAllowed('https://chatgpt.com/connector/oauth/callback', CHATGPT_OAUTH_REDIRECT_URIS),
      true
    );
  });

  it('rejects evil.com and suffix domains and userinfo tricks', () => {
    assert.equal(isRedirectUriAllowed('https://evil.com/cb', CHATGPT_OAUTH_REDIRECT_URIS), false);
    assert.equal(
      isRedirectUriAllowed('https://chatgpt.com.evil.com/callback', CHATGPT_OAUTH_REDIRECT_URIS),
      false
    );
    assert.equal(
      isRedirectUriAllowed('https://chatgpt.com@evil.com/callback', CHATGPT_OAUTH_REDIRECT_URIS),
...
describe('scopes', () => {
  ...
  it('fails closed on missing scopes', () => {
    const result = hasRequiredScopes([MCP_SCOPES.READ], [MCP_SCOPES.WRITE, MCP_SCOPES.TOOLS]);
