...
const {
  CANONICAL_SOCIAL_MCP_TOOLS,
  SOCIAL_PUBLISH_TOOL_CATALOG_VERSION,
...
const { MCP_TOOL_CATALOG_VERSION } = await import('../../src/lib/mcp/standardResponse.ts');
const { CHATGPT_CONNECTOR_TOOL_NAMES } = await import('../../src/lib/mcp/toolAnnotations.ts');
const {
  initializeRegistry,
  listTools,
  hasTool,
...
test('tools/list exposes every canonical social tool', async () => {
  ...
  for (const required of CANONICAL_SOCIAL_MCP_TOOLS) {
    ...
    assert.ok(hasTool(required) || names.has(required), `not callable: ${required}`);
  }
});
...
test('inspect_tools catalog (full) and tools/list include the same canonical social tools', async () => {
  initializeRegistry();
  invalidateUnifiedMcpToolCache();
  const full = await getUnifiedMcpTools({ forceRefresh: true, forChatGPT: false });
  ...
  for (const name of CANONICAL_SOCIAL_MCP_TOOLS) {
    ...
    assert.ok(chatgptNames.has(name), `ChatGPT curated catalog missing ${name}`);
  }
});
...
test('organization posting never falls back to personal — identity type enforced', async () => {
  ...
  const { SocialPublishingService } = await import(
    '../../src/lib/social/SocialPublishingService.ts'
  );
  const service = new SocialPublishingService();
  const result = await service.publishToLinkedIn('00000000-0000-0000-0000-000000000099', {
    platform: 'linkedin',
...
test('LinkedIn identities response shape contract (org != scopes)', () => {
  ...
  assert.ok(sample.organizations[0].organization_urn.startsWith('urn:li:organization:'));
  // Scopes alone must never appear as an organization identity
  assert.equal(
    sample.organizations.some((o) => o.organization_id === 'w_organization_social'),
    false
  );
});

test('verification detects missing provider posts (Facebook helper)', async () => {
  const { verifyFacebookPostExists, FacebookPublishError } = await import(
    '../../src/lib/facebook/verifyFacebookPost.ts'
  );
