...
describe("mcp oauth www-authenticate", () => {
  it("includes resource_metadata for ChatGPT OAuth discovery", async () => {
    const { createWWWAuthenticateHeader } =
      await import("../../src/services/mcp/authMiddlewareApp.ts");
    ...
    assert.match(
      header,
      /resource_metadata="https:\/\/alphaclonesystems\.com\/\.well-known\/oauth-protected-resource"/,
    );
  ...
  it("allows ChatGPT Apps redirect URI patterns", async () => {
    const { isRedirectUriAllowed, CHATGPT_OAUTH_REDIRECT_URIS } =
