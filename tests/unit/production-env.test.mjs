...
test('rejects missing critical values without returning their contents', () => {
  ...
  assert.equal(result.errors.some((error) => error.includes('undefined')), false);
});
...
test('rejects local, insecure, and Vercel production URLs', () => {
  for (const appUrl of [
    ...
    const result = validateProductionEnv(validEnv({ NEXT_PUBLIC_APP_URL: appUrl }));
    assert.equal(result.ok, false, appUrl);
  }
});

test('rejects an invalid encryption-secret length', () => {
  const result = validateProductionEnv(validEnv({ ENCRYPTION_SECRET: 'too-short' }));
  assert.equal(result.ok, false);
