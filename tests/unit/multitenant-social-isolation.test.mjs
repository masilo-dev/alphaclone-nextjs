...
test("resolveTrustedTenantId rejects missing tenant", () => {
  assert.throws(
    ...
    (err) =>
      err instanceof TenantIsolationError && err.code === "TENANT_REQUIRED",
  );
});
...
test("getFacebookIntegration requires tenantId", async () => {
  const { getFacebookIntegration } =
    await import("../../src/services/facebook/facebookIntegrationService.ts");
...
test("canonical tools include get_social_identities", async () => {
  const { CANONICAL_SOCIAL_MCP_TOOLS } =
