...
import { DEPARTMENT_AGENTS, getAgentById, toOrchestratorSubagents } from '../../src/lib/bonnie/os/agentRegistry.ts';
import {
...
import { BONNIE_EVENT_GOALS, isBonnieReasoningEvent } from '../../src/lib/bonnie/os/eventReasoning.ts';
...
test('department agent roster includes required specialists', () => {
  ...
  for (const required of [
    'ceo', 'coo', 'sales', 'crm', 'marketing', 'social', 'finance', 'accounting',
    'research', 'email', 'calendar', 'document', 'customer_success', 'support',
    'compliance', 'security', 'reporting', 'workflow', 'automation', 'knowledge',
    'supervisor', 'audit', 'memory', 'evaluation',
  ]) {
...
test('supervisor selects finance agents for overdue invoice goals', () => {
  const agents = selectAgentsForGoal('Chase overdue invoices and improve cash collection', { maxAgents: 4 });
  const ids = agents.map((a) => a.id);
  assert.ok(ids.includes('finance') || ids.includes('accounting'));
  const decision = decideSupervision({ goal: 'Send overdue invoice reminders in bulk', selectedAgents: agents });
...
test('supervisor maps lead_created events to research/crm/sales', () => {
  const agents = selectAgentsForGoal('New lead arrived', { eventType: 'lead_created', maxAgents: 4 });
...
test('estimateRiskLevel and tool collection', () => {
  ...
  const tools = collectToolsFromAgents([getAgentById('crm'), getAgentById('finance')].filter(Boolean));
...
test('reflection synthesizes lessons and memory updates', () => {
  const supervisor = decideSupervision({ goal: 'Qualify leads and draft outreach' });
...
test('complex mission detector still triggers cognitive path', () => {
  assert.equal(
    looksLikeComplexMission('Create contact, send outreach, and then schedule follow-up after that'),
    true
  );
