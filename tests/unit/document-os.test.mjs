...
test("Novus Power conflicting originality clauses are blocking", () => {
  ...
  assert.ok(
    ipIssues.some(
      (i) => i.code.includes("CONTRADICTION") || i.code.includes("IP_"),
    ),
  );
  ...
  assert.ok(
    result.blocking_issues.some(
      (i) =>
        i.code === "JURISDICTION_UNDEFINED" ||
        i.code === "GOVERNING_LAW_UNDEFINED",
    ),
  );
  assert.ok(
    result.blocking_issues.some(
      (i) =>
        i.code === "CLIENT_EMAIL_INCONSISTENT" ||
        i.code === "CONTACT_EMAIL_INCONSISTENT",
    ),
  );
});
...
test("Novus Power coherent contract can be approved and does not include invoice total box", () => {
  ...
  assert.equal(
    result.valid,
    true,
    JSON.stringify(result.blocking_issues, null, 2),
  );
...
test("legacy themed renderer also omits invoice total box on contracts", () => {
  const html = renderDocumentHtml({
    ...
    branding: {
      name: brand.legal_business_name,
      logoUrl: brand.primary_logo_url,
      primaryColor: brand.primary_colour,
    },
    total: 2500,
    status: "draft",
...
test("unsigned drafts must not contain signatures; signed contracts are not draft", () => {
  ...
  assert.ok(
    layoutIssues.some((i) => i.code === "UNSIGNED_DRAFT_HAS_SIGNATURES"),
  );
...
test("paid Novus invoice shows zero balance with payment evidence", () => {
  ...
  assert.ok(
    badIssues.some(
      (i) =>
        i.code === "PAID_NONZERO_BALANCE" ||
        i.code === "PAID_WITH_TOTAL_DUE" ||
        i.code === "BALANCE_EQUATION",
    ),
  );
});
...
test("AI cannot send contracts without owner approval", () => {
  ...
  assert.throws(() =>
    requireOwnerApprovalForSensitiveAction("send_contract", ai, false),
  );
