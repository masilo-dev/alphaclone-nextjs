...
test("conversations API supports list + create actions", () => {
  const src = fs.readFileSync(
    path.join(root, "src/app/api/bonnie/conversations/route.ts"),
    "utf8",
  );
...
test("conversation patch supports pin/archive/rename", () => {
  const src = fs.readFileSync(
    path.join(root, "src/app/api/bonnie/conversations/[id]/route.ts"),
    "utf8",
  );
...
test("BonnieFullView mounts workspace shell pieces", () => {
  const src = fs.readFileSync(
    path.join(root, "src/components/dashboard/bonnie/BonnieFullView.tsx"),
    "utf8",
  );
...
test("Bonnie welcome headline exists", () => {
  ...
  assert.match(src, /What should Bonnie execute\?/);
});

test("Chat panel supports stop generation and activity cards", () => {
  const src = fs.readFileSync(
    path.join(root, "src/components/dashboard/bonnie/BonnieChatPanel.tsx"),
    "utf8",
  );
