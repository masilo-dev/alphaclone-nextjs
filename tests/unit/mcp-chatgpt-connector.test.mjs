...
describe("mcp connector helpers", () => {
  it("normalizes pagination with defaults and caps", async () => {
    const { normalizePagination, buildPaginationMeta } =
      await import("../../src/lib/mcp/connector/pagination.ts");
  ...
  it("builds structured success and error envelopes", async () => {
    const { okResult, errorResult, toMcpContent } =
      await import("../../src/lib/mcp/connector/response.ts");
  ...
  it("includes connector tools in chatgpt-app-submission.json", () => {
    const doc = JSON.parse(
      fs.readFileSync(
        path.join(process.cwd(), "chatgpt-app-submission.json"),
        "utf8",
      ),
    );
    const missing = REQUIRED_CONNECTOR_TOOLS.filter(
      (name) => !doc.tools?.[name],
    );
    assert.deepEqual(
      missing,
      [],
      `Missing ChatGPT annotations: ${missing.join(", ")}`,
    );
  });

  it("attaches OpenAI-required annotations on unified tools/list output", () => {
    const listSrc = fs.readFileSync(
      path.join(process.cwd(), "src/lib/mcp/listAllTools.ts"),
      "utf8",
    );
    const annSrc = fs.readFileSync(
      path.join(process.cwd(), "src/lib/mcp/toolAnnotations.ts"),
      "utf8",
    );
    ...
    const aliasSrc = fs.readFileSync(
      path.join(process.cwd(), "src/lib/mcp/tools/chatgpt-aliases.ts"),
      "utf8",
    );
  ...
  it("documents deployment and ChatGPT compatibility", () => {
    ...
    assert.ok(
      fs.existsSync(path.join(process.cwd(), "docker-compose.mcp.yml")),
    );
  });
});
