  ...
  if (selected) {
    return (
      <div className="p-4 space-y-4 overflow-y-auto pb-24">
        <button onClick={() => setSelected(null)} className="type-ui text-teal-400 font-bold">
          ← Back
        </button>
      ...
      <div className="bg-slate-900 border border-white/5 rounded-2xl p-4 space-y-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1 type-caption font-bold text-teal-400"
          >
            <Plus className="w-3.5 h-3.5" /> Add step
          </button>
          <button onClick={saveSequence} className="ml-auto h-9 px-4 rounded-xl bg-teal-500 text-white type-ui font-bold">
            Save sequence
          </button>
        </div>
      </div>
