...
export default function TaskScheduler({ onTaskComplete }: { onTaskComplete?: (task: ScheduledTask) => void }) {
  ...
  return <div className="space-y-5">
    <div className="flex items-start justify-between gap-4">
      <div><h2 className="text-2xl font-semibold text-white flex items-center gap-2"><Bot className="w-6 h-6 text-violet-400" /> AI Task Scheduler</h2><p className="type-card-description text-slate-400 mt-1">Run durable AI analysis and drafting tasks on a UTC schedule. Every run stores its real provider result.</p></div>
    ...
    {loading ? <div className="py-12 flex justify-center"><Loader2 className="w-6 h-6 animate-spin text-violet-400" /></div> : tasks.length === 0 ? <div className="dashboard-panel-soft p-10 text-center"><CalendarClock className="w-9 h-9 text-slate-600 mx-auto mb-3" /><p className="text-white font-medium">No scheduled AI tasks</p><p className="type-card-description text-slate-500 mt-1">Create one to generate a stored result on a durable server schedule.</p></div> : <div className="space-y-3">{tasks.map((task) => <div key={task.id} className="dashboard-panel-soft p-4">
      ...
      {expandedId === task.id && <div className="mt-4 pt-4 border-t border-slate-800 space-y-3"><div><p className="type-caption uppercase tracking-wide text-slate-500 mb-1">Instructions</p><p className="type-caption text-slate-300 whitespace-pre-wrap">{task.prompt}</p></div><div><p className="type-caption uppercase tracking-wide text-slate-500 mb-1">Latest run</p>{task.latest_result ? <div className={`rounded-lg p-3 type-caption whitespace-pre-wrap ${task.latest_result.status === 'success' ? 'bg-emerald-500/5 text-slate-300' : 'bg-rose-500/5 text-rose-300'}`}>{task.latest_result.output || task.latest_result.error || 'No output was recorded.'}<p className="type-caption text-slate-600 mt-2">{new Date(task.latest_result.ran_at).toLocaleString()}</p></div> : <p className="type-caption text-slate-500">This task has not run yet.</p>}</div></div>}
    </div>)}</div>}
  </div>;
}
