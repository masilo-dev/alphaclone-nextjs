      ...
      <div className="w-full max-w-lg overflow-hidden rounded-3xl border border-slate-700/80 bg-slate-950 shadow-2xl shadow-black/60" role="dialog" aria-modal="true" aria-labelledby="client-portal-access-title">
        <div className="border-b border-slate-800 bg-gradient-to-br from-cyan-950/40 to-slate-950 p-5 sm:p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="type-caption font-bold uppercase tracking-caps text-cyan-300">Client handoff</p>
                <h2 id="client-portal-access-title" className="mt-1 text-lg font-bold text-white">Set up {client.name}&apos;s portal</h2>
                <p className="mt-1 type-card-description leading-relaxed text-slate-400">Create secure access, then copy the exact details to send to your client.</p>
              </div>
            </div>
        ...
        <div className="space-y-4 p-5 sm:p-6">
          ...
          <div>
            ...
            <p className="mt-1.5 type-card-description text-amber-300/80">Share this password through a secure channel. It is never stored in the browser URL.</p>
          </div>

          <div className="flex items-start gap-2.5 rounded-2xl border border-slate-800 bg-slate-900/60 p-3 type-caption leading-relaxed text-slate-400">
          ...
          {!portalUrl ? (
            <button type="button" onClick={grantAccess} disabled={saving} className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-500 px-4 py-3 type-ui font-bold text-slate-950 transition-colors hover:bg-cyan-300 disabled:cursor-wait disabled:opacity-60">
            ...
            <div className="space-y-3 rounded-2xl border border-emerald-400/20 bg-emerald-400/5 p-4">
              <div className="flex items-center gap-2 type-ui font-bold text-emerald-300"><Check className="h-4 w-4" aria-hidden="true" /> Access is ready</div>
              <div className="rounded-xl border border-slate-700 bg-slate-950 p-3 type-caption text-slate-300">
              ...
              <div className="flex flex-col gap-2 sm:flex-row">
                <button type="button" onClick={copyHandoff} className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-emerald-400 px-4 py-2.5 type-ui font-bold text-slate-950 hover:bg-emerald-300">
                  {copied ? <Check className="h-4 w-4" /> : <Clipboard className="h-4 w-4" />}
                  {copied ? 'Copied' : 'Copy handoff details'}
                </button>
                <a href={portalUrl} target="_blank" rel="noreferrer" className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-slate-700 px-4 py-2.5 type-ui font-bold text-slate-200 hover:border-cyan-400/40 hover:text-white">
                  <ExternalLink className="h-4 w-4" /> Open portal
                </a>
              </div>
              <p className="type-card-description leading-relaxed text-slate-500">For security, ask the client to change this password after their first sign-in. If you need to reset it, create a new password here.</p>
            </div>
          )}
        </div>
      </div>
