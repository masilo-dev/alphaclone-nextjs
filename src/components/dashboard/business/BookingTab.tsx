        ...
        <div className="space-y-6 pb-20 ac-scroll-full ac-enterprise-module">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-400 type-card-description">Manage your meetings and availability</p>
                </div>

                <div className="flex bg-slate-900 p-1 rounded-xl border border-slate-800 self-stretch sm:self-auto">
                    <button
                        onClick={() => setActiveView('schedule')}
                        className={`flex-1 sm:flex-none px-4 py-2 rounded-lg type-ui font-bold transition-all ${activeView === 'schedule' ? 'bg-teal-500 text-slate-950' : 'text-slate-400 hover:text-white'}`}
                    >
                        My Schedule
                    </button>
                    <button
                        onClick={() => setActiveView('booking')}
                        className={`flex-1 sm:flex-none px-4 py-2 rounded-lg type-ui font-bold transition-all ${activeView === 'booking' ? 'bg-teal-500 text-slate-950' : 'text-slate-400 hover:text-white'}`}
                    >
                        Booking Page
                    </button>
                </div>
            ...
            {activeView === 'schedule' ? (
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <h3 className="type-caption font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="flex items-center gap-1.5 type-caption font-bold text-slate-300 hover:text-white transition-colors bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-700"
                            >
                            ...
                            <button
                                ...
                                disabled={syncing}
                                className="flex items-center gap-1.5 type-caption font-bold text-teal-400 hover:text-teal-300 transition-colors bg-teal-500/5 px-3 py-1.5 rounded-lg border border-teal-500/20 disabled:opacity-50"
                            >
                        ...
                        <>
                            <Card className="bg-slate-900/60 border-slate-800 p-4 mb-6 flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="type-card-description text-slate-400 leading-relaxed mb-3">
                                    ...
                                    <a
                                        ...
                                        className="type-caption font-bold text-teal-400 hover:text-teal-300 flex items-center gap-1 w-max"
                                    >
                            ...
                            {scheduledEvents.length > 0 ? (
                                            ...
                                            <Card key={event.id} className="p-5 bg-slate-900/40 border-slate-800 hover:border-teal-500/30 transition-all flex flex-col justify-between group">
                                                <div>
                                                    <div className="flex justify-between items-start mb-3">
                                                        ...
                                                        <span className="type-caption font-bold px-2 py-1 bg-slate-800 text-slate-400 rounded-lg border border-slate-700">
                                                            Scheduled
                                                        </span>
                                                    </div>
                                                    <h4 className="font-bold text-white mb-1 group-hover:text-teal-400 transition-colors line-clamp-1">{event.title}</h4>
                                                    <div className="flex items-center gap-2 type-caption text-slate-500 mb-4">
                                                    ...
                                                    {invitee && (
                                                        <div className="bg-slate-950/50 rounded-xl p-3 mb-4 space-y-2 border border-slate-800/50">
                                                            <div className="flex items-center justify-between">
                                                                <span className="type-caption text-slate-500">Participant</span>
                                                                <span className="type-caption font-medium text-slate-200">{invitee.name}</span>
                                                            </div>
                                                            <div className="flex items-center justify-between">
                                                                <span className="type-caption text-slate-500">Email</span>
                                                                <span className="type-caption text-slate-300">{invitee.email}</span>
                                                            </div>
                                                            {invitee.questions_and_responses && invitee.questions_and_responses.length > 0 && (
                                                                <div className="pt-2 mt-2 border-t border-slate-800/50">
                                                                    <span className="type-caption text-slate-500 mb-1 block">Notes / Answers</span>
                                                                    <p className="type-card-description text-slate-300 line-clamp-2">
                                                ...
                                                <div className="pt-4 border-t border-slate-800 flex flex-col gap-3">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            {invitee.reschedule_url && (
                                                                <button
                                                                    ...
                                                                    className="flex-1 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 type-caption font-bold uppercase tracking-wide rounded-lg transition-colors text-center"
                                                                >
                                                                    Reschedule
                                                                </button>
                                                            )}
                                                            {invitee.cancel_url && (
                                                                <button
                                                                    ...
                                                                    className="flex-1 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 type-caption font-bold uppercase tracking-wide rounded-lg transition-colors text-center"
                                                                >
                                                                    Cancel
                                                                </button>
                                                            )}
                                                    ...
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="type-caption text-slate-400">1 Invitee</span>
                                                        </div>
                                                        <button
                                ...
                                <div className="flex flex-col items-center justify-center py-16 bg-slate-900/20 border border-slate-800 border-dashed rounded-3xl">
                                    ...
                                    <p className="text-slate-500 font-medium type-card-description">No upcoming appointments found.</p>
                    ...
                    <div className="flex items-center justify-between bg-teal-500/5 border border-teal-500/10 p-4 rounded-2xl">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 type-caption font-black uppercase tracking-wider rounded-lg transition-all"
                            >
                                <ExternalLink className="w-3 h-3" />
                                Open Calendly Admin
                            </button>
