...
export default function ResendIntegration() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.error(err.message || 'Failed to save Resend integration');
        } finally {
            setIsSaving(false);
        }
    };
        ...
        try {
            const res = await fetch('/api/email/providers/test', {
                ...
                body: JSON.stringify({
                    ...
                    message: 'Your Resend integration is ready for campaigns and outreach.',
                }),
            });
            const data = await res.json().catch(() => ({}));
            if (!res.ok || !data.success) {
                throw new Error(data.error || 'Resend test failed');
            }
            toast.success('Resend test email sent successfully');
        } catch (err: any) {
            toast.error(err.message || 'Resend test failed');
        } finally {
            setIsTesting(false);
        }
    };
...
>>>>>>> origin/main
                ...
                <div className="flex items-center gap-3 pt-4 border-t border-white/5">
                    <Button 
                        type="submit" 
                        disabled={isSaving}
                        className="bg-zinc-800 hover:bg-zinc-700 text-white font-bold px-8 shadow-[0_0_20px_-5px_rgba(255,255,255,0.1)]"
                    >
