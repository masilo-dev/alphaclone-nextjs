...
export default function UnifiedInboxTab({
    ...
    contactSearchTimeout.current = setTimeout(async () => {
      try {
        const { data } = await supabase
          ...
          .limit(8);
        if (data && data.length > 0) {
          setContactResults(data.map((c: any) => ({ id: c.id, name: c.full_name || "", email: c.email || "" })));
  ...
  const handleSelectContact = (c: { name: string; email: string }) => {
  ...
  const handleSendCompose = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/outreach/send", {
        ...
        body: JSON.stringify({
          tenantId: tenant.id,
          leadEmail: composeTo.trim(),
          leadName: composeToName || undefined,
      ...
      toast.success(`Sent via ${String(data.provider || provider || "platform").toUpperCase()}`, { id: t });
      setComposing(false);
      setComposeTo(""); setComposeToName(""); setContactSearch("");
