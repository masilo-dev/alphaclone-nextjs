            ...
            return (
                <div className="fixed inset-0 bg-slate-950 flex items-center justify-center z-50 overflow-hidden p-4">
                    <div className="w-full max-w-xl rounded-2xl border border-white/10 bg-slate-900/80 p-6 shadow-2xl">
                        ...
                        <p className="text-slate-400 type-card-description mb-5">
                            Complete a quick device check, then join the full-screen meeting room. You can share your screen once connected.
                        </p>
                        {preJoinError && (
                            <div className="mb-4 rounded-lg border border-red-500/30 bg-red-500/10 p-3 type-ui text-red-300">
                                {preJoinError}
                            </div>
                        )}
                        <div className="flex flex-wrap gap-3">
                            <button
                                ...
                                className="px-4 py-2 rounded-xl border border-teal-500/30 text-teal-300 hover:bg-teal-500/10 transition-colors type-ui font-semibold disabled:opacity-60"
                            >
                                {isCheckingDevices ? 'Checking devices...' : 'Check camera and mic'}
                            </button>
                            <button
                                onClick={() => setPreJoinAccepted(true)}
                                className="px-4 py-2 rounded-xl bg-teal-600 hover:bg-teal-500 text-white transition-colors type-ui font-semibold"
                            >
                                Join now
                            </button>
                            <button
                                onClick={() => void handleLeave()}
                                className="px-4 py-2 rounded-xl border border-white/10 text-slate-300 hover:bg-white/5 transition-colors type-ui font-semibold"
                            >
                                Cancel
                            </button>
                        </div>
        ...
        return (
            <div className="fixed bottom-24 right-4 z-[120] w-[360px] max-w-[calc(100vw-2rem)] rounded-2xl border border-white/10 bg-slate-950/95 shadow-2xl overflow-hidden backdrop-blur-md">
                <div className="flex items-center justify-between px-3 py-2 bg-slate-900/90 border-b border-white/10">
                    <div className="flex items-center gap-2 type-caption font-semibold text-slate-200">
                ...
                <div className="h-[200px] bg-slate-900">
                    {primaryParticipant ? (
                        ...
                        <div className="h-full w-full flex items-center justify-center type-ui text-slate-400">
            ...
            <header className="absolute top-[env(safe-area-inset-top)] left-0 right-0 h-14 sm:h-16 bg-gradient-to-b from-black/70 to-transparent z-[110] px-3 sm:px-6 flex items-center justify-between pointer-events-none">
                <div className="flex items-center gap-4 pointer-events-auto">
                    <div className="flex items-center gap-2 bg-slate-900/60 backdrop-blur-md border border-white/10 px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-2xl">
                        ...
                        <span className="type-caption sm:text-xs font-bold uppercase tracking-widest text-white/90">
                ...
                <div className="flex items-center gap-1.5 sm:gap-2 pointer-events-auto">
                    <div className={`px-2 sm:px-3 py-1 rounded-xl border type-caption sm:text-xs font-bold uppercase tracking-wide ${
                        networkQuality === 'good'
                    ...
                    {(platformState === 'error' || networkQuality === 'poor') && !isMobile && (
                        <button
                            ...
                            className="px-3 py-1 rounded-xl border border-white/10 type-caption text-slate-200 hover:bg-white/10 transition-colors"
                            title="Reconnect"
                        >
                    ...
                    <div className={`flex bg-slate-900/40 backdrop-blur-xl rounded-2xl border border-white/10 p-0.5 sm:p-1 ${isMobile ? 'hidden' : ''}`}>
                        <button
                            ...
                            className={`px-4 py-2 rounded-xl type-caption font-bold transition-all ${viewMode === 'grid' ? 'bg-white text-black shadow-lg' : 'text-slate-400 hover:text-white'}`}
                        >
                            Grid
                        </button>
                        <button
                            onClick={() => setViewMode('speaker')}
                            className={`px-4 py-2 rounded-xl type-caption font-bold transition-all ${viewMode === 'speaker' ? 'bg-white text-black shadow-lg' : 'text-slate-400 hover:text-white'}`}
                        >
                            Speaker
                        </button>
                    </div>
                        ...
                        {participants.map(p => (
                            <div key={p.sessionId} className="relative rounded-3xl overflow-hidden bg-slate-900 ring-1 ring-white/5 shadow-2xl transition-transform duration-500">
                                ...
                                <div className="absolute bottom-4 left-4 flex items-center gap-2 bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/10">
                                    ...
                                    <span className="type-caption font-semibold">{p.isLocal ? "You" : p.userName}</span>
                                </div>
                            </div>
                        ))}
                    </div>
