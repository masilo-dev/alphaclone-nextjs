        ...
        <div className="flex flex-col h-full">
            ...
            <div className="p-4 sm:p-6">
                <button 
                    ...
                    className="w-full flex items-center justify-center gap-3 bg-blue-600 hover:bg-blue-500 text-white py-4 px-4 rounded-2xl transition-all shadow-xl active:scale-95 group font-black uppercase text-xs"
                >
                    <Plus size={20} /> 
                    Compose
                </button>
            ...
            <nav className="flex-1 overflow-y-auto px-4 pb-4 space-y-1.5 custom-scrollbar">
                ...
                {folders.map(folder => (
                    <button
                        ...
                        className={`w-full flex items-center justify-between px-4 py-3.5 rounded-2xl transition-all group ${selectedFolder === folder.folderId ? 'bg-blue-600/10 text-white border border-blue-500/20' : 'text-gray-500 hover:bg-white/5 hover:text-gray-300'}`}
                    >
                        ...
                        {folder.unreadCount > 0 && (
                            <span className="text-[10px] px-2 py-0.5 rounded-full font-black bg-blue-600 text-white">{folder.unreadCount}</span>
                        )}
                    </button>
                ))}
            </nav>
            ...
            <div className="p-4 border-t border-white/5 space-y-2">
                <button onClick={() => setIsLeadModalOpen(true)} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-gray-500 hover:bg-white/5 transition-all text-sm font-bold">
                    <Sparkles size={18} className="text-blue-500" /> Lead Outreach
                </button>
            </div>
        </div>
    );
                    ...
                    <div className="h-20 border-b border-white/5 px-6 flex items-center gap-4 sticky top-0 z-10 bg-[#0f0f0f]/80 backdrop-blur-md">
                        ...
                        <div className="relative flex-1">
                            ...
                            <input 
                                ...
                                className="w-full bg-black/40 border border-white/5 rounded-2xl pl-12 pr-4 py-3 text-sm text-white focus:border-blue-500/50 outline-none"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-1">
                        ...
                        <div className="flex gap-2 p-2 overflow-x-auto no-scrollbar">
                            {['all', 'urgent', 'follow-up', 'newsletter'].map(cat => (
                                <button key={cat} onClick={() => setCategoryFilter(cat as any)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${categoryFilter === cat ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-600/20' : 'bg-white/5 border-white/5 text-gray-600'}`}>
                                    {cat}
                                </button>
                            ))}
                        </div>
                        ...
                        {loading && messages.length === 0 ? (
                            ...
                            filteredMessages.map(msg => (
                                <button
                                    ...
                                    className={`w-full text-left p-4 rounded-2xl transition-all border ${selectedMessage === msg.messageId ? 'bg-blue-600/10 border-blue-500/30' : 'bg-transparent border-transparent hover:bg-white/5'}`}
                                >
                                    ...
                                    <p className={`text-sm font-bold truncate mb-1 ${msg.status === 'unread' ? 'text-blue-400' : 'text-gray-400'}`}>{msg.subject}</p>
                        ...
                        {composing ? (
                                ...
                                <form onSubmit={handleSend} className="space-y-6 max-w-4xl">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-gray-600 uppercase tracking-widest px-2">Recipient</label>
                                            <input type="email" required value={emailData.to} onChange={e => setEmailData({...emailData, to: e.target.value})} className="w-full h-14 bg-black/40 border border-white/5 rounded-2xl px-5 text-base text-white outline-none focus:border-blue-500/50" placeholder="name@example.com" />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black text-gray-600 uppercase tracking-widest px-2">Subject</label>
                                            <input type="text" required value={emailData.subject} onChange={e => setEmailData({...emailData, subject: e.target.value})} className="w-full h-14 bg-black/40 border border-white/5 rounded-2xl px-5 text-base text-white outline-none focus:border-blue-500/50" placeholder="Topic" />
                                        </div>
                                    </div>
                                    <div className="space-y-2">
                                        <div className="flex justify-between items-center px-2">
                                            <label className="text-[10px] font-black text-gray-600 uppercase tracking-widest">Message</label>
                                            <button type="button" onClick={() => setShowAiPrompt(!showAiPrompt)} className="flex items-center gap-2 text-[10px] font-black text-blue-500 uppercase tracking-widest bg-blue-500/10 px-3 py-1.5 rounded-lg border border-blue-500/20">
                                                <Sparkles size={12} /> AI Assist
                                            </button>
                                        </div>
                                        <textarea required rows={10} value={emailData.body} onChange={e => setEmailData({...emailData, body: e.target.value})} className="w-full bg-black/40 border border-white/5 rounded-3xl px-6 py-6 text-lg text-white outline-none focus:border-blue-500/50 min-h-[300px] resize-none" placeholder="Start typing..." />
                                    </div>
                                    <div className="flex justify-end pt-6 sticky bottom-0 bg-[#141414] py-4 sm:static sm:bg-transparent">
                                        <button disabled={sending} type="submit" className="w-full sm:w-auto px-12 py-5 bg-blue-600 text-white font-black uppercase text-sm rounded-2xl shadow-2xl shadow-blue-900/40 active:scale-95 transition-all">
                            ...
                            <motion.div key="content" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col h-full overflow-hidden">
                                <div className="h-20 border-b border-white/5 px-4 sm:px-8 flex items-center justify-between bg-[#141414]/80 backdrop-blur-md sticky top-0 z-10 shrink-0">
                                    <div className="flex items-center gap-4 min-w-0">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-[10px] text-blue-400 font-bold truncate lowercase">{messageContent?.sender}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        {!isMobile && (
                                            <button onClick={() => handleAiReply()} disabled={aiGenerating} className="flex items-center gap-2 bg-blue-600/10 text-blue-400 px-4 py-2.5 rounded-xl border border-blue-600/20 text-[10px] font-black uppercase tracking-widest">
                                    ...
                                    <div className="max-w-4xl mx-auto space-y-10">
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-8 border-b border-white/5">
                                            <div className="flex gap-4">
                                                <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center font-black text-blue-500 border border-white/5 shadow-inner text-xl">
                                            ...
                                            {isMobile && (
                                                <button onClick={() => handleAiReply()} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs shadow-lg">AI Reply Assist</button>
                                            )}
                                        </div>
                                ...
                                {isMobile && selectedMessage && (
                                    <div className="fixed bottom-0 left-0 right-0 p-4 bg-[#0a0a0a]/90 backdrop-blur-xl border-t border-white/10 z-50 flex gap-3">
                                        <input 
                                            value={replyBody}
                                            onChange={e => setReplyBody(e.target.value)}
                                            placeholder="Quick reply..." 
                                            className="flex-1 bg-black border border-white/10 rounded-xl px-4 text-sm text-white"
                                        />
                                        <button className="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center text-white"><Send size={20} /></button>
                                    </div>
                                )}
                            </motion.div>
                        ) : (
