    ...
    if (loading) {
        return (
            <div className="flex flex-col justify-center items-center h-96 text-slate-400 gap-4">
                ...
                <p className="font-medium uppercase tracking-widest text-[10px]">Syncing Ledger...</p>
            </div>
        );
    }
        ...
        <div className={`space-y-6 max-w-7xl mx-auto pb-24 ${isMobile ? 'px-2' : 'px-6'}`}>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                        <span className="text-[10px] font-black text-teal-500 uppercase tracking-widest bg-teal-500/10 px-2 py-0.5 rounded">Professional Edition</span>
            ...
            <div className="flex border-b border-white/5 overflow-x-auto no-scrollbar">
                {(['overview', 'income', 'balance'] as const).map(tab => (
                    <button
                        ...
                        className={`px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] whitespace-nowrap transition-all border-b-2 ${activeTab === tab ? 'border-teal-500 text-white bg-teal-500/5' : 'border-transparent text-gray-500'}`}
                    >
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <Card className="p-6 bg-slate-900/40 border-white/5">
                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">Available Cash</p>
                            <h3 className="text-2xl font-black text-white">${stats.cashBalance.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
                        </Card>
                        <Card className="p-6 bg-slate-900/40 border-white/5">
                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">Revenue (MTD)</p>
                            <h3 className="text-2xl font-black text-teal-400">${stats.totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2 })}</h3>
                        </Card>
                        <Card className="p-6 bg-slate-900/40 border-white/5">
                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">Expenses (MTD)</p>
                    ...
                    <div className="bg-slate-900/40 border border-white/5 rounded-2xl overflow-hidden">
                        <div className="p-5 border-b border-white/5 flex justify-between items-center bg-[#141414]">
                            <h3 className="text-[10px] font-black text-white uppercase tracking-widest flex items-center"><Activity size={16} className="mr-2 text-teal-500" /> Recent Ledger Activity</h3>
                        </div>
            ...
            {activeTab === 'income' && (
                <Card className="p-6 sm:p-10 bg-slate-900/60 border-white/5 animate-in fade-in duration-300">
                    ...
                    <div className="space-y-8">
                        <div>
                            <div className="text-[10px] font-black text-teal-500 uppercase tracking-widest mb-4 border-b border-teal-500/20 pb-2">Operating Revenue</div>
                        ...
                        <div>
                            <div className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-4 border-b border-rose-500/20 pb-2">Operating Expenses</div>
            ...
            {activeTab === 'balance' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in duration-300">
                    <Card className="p-6 bg-slate-900/60 border-white/5">
                        <div className="text-[10px] font-black text-teal-500 uppercase tracking-widest mb-6 border-b border-teal-500/20 pb-2">Assets</div>
                    ...
                    <Card className="p-6 bg-slate-900/60 border-white/5">
                        <div className="text-[10px] font-black text-rose-500 uppercase tracking-widest mb-6 border-b border-rose-500/20 pb-2">Liabilities & Equity</div>
                        <div className="space-y-4">
            ...
            {isMobile && (
                <div className="fixed bottom-0 left-0 right-0 p-4 bg-black/80 backdrop-blur-xl border-t border-white/5 z-50 flex gap-2">
                    <button onClick={() => setIsUploadOpen(true)} className="flex-1 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-white text-[10px] font-black uppercase tracking-widest"><Upload size={18} className="mr-2" /> Upload</button>
                    <button onClick={() => setIsReceiptGeneratorOpen(true)} className="flex-1 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-white text-[10px] font-black uppercase tracking-widest"><FileText size={18} className="mr-2" /> Receipt</button>
                </div>
            )}
        </div>
    );
