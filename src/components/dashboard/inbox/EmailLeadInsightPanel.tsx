    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <p className="type-caption font-bold uppercase tracking-wide text-teal-300">
            Auto lead search
          </p>
        ...
        {collapsible && (
          <button
            ...
            className="type-ui font-bold text-teal-400 hover:text-teal-300 flex items-center gap-1"
          >
      ...
      {error && <p className="type-card-description text-amber-400 mb-2">{error}</p>}
            ...
            <ul className="space-y-1.5 mb-2">
              {result.matches.slice(0, compact ? 3 : 6).map((m) => (
                <li key={`${m.type}-${m.id}`}>
                  <Link
                    href={m.href}
                    className="flex items-center gap-2 type-caption text-slate-200 hover:text-teal-300 transition-colors"
                  >
                    {m.type === 'account' ? (
                    ...
                    <span className="type-ui text-slate-500 shrink-0">
                      {matchLabel(m)}
          ...
          <div className="flex flex-wrap gap-2">
            {result.suggestedActions.map((action) => (
              <Link
                key={action.href}
                href={action.href}
                className="inline-flex items-center gap-1 rounded-lg border border-teal-500/30 px-2.5 py-1 type-ui font-semibold text-teal-300 hover:bg-teal-500/10"
              >
