...
function cleanupChannelsByTopic(topic: string) {
  const existing = supabase
    .getChannels()
    .filter((channel: RealtimeChannel) => channel.topic === topic);
...
const MessagesTab: React.FC<MessagesTabProps> = ({
  ...
  const [activeChannel, setActiveChannel] = useState<"all" | "email" | "chat">(
    "all",
  );
  const [isAIGenerating, setIsAIGenerating] = useState(false);
  const { currentTenant } = useTenant();
  const { clients: crmClients, isLoading: isLoadingCRM } = useClients(
    currentTenant?.id,
    { limit: 100 },
  );
  ...
  const [pendingAttachments, setPendingAttachments] = useState<
    { id: string; url: string; type: "image" | "file"; name: string }[]
  >([]);
  const [isUploading, setIsUploading] = useState(false);
  const [typingUsers, setTypingUsers] = useState<Set<string>>(new Set());
  const [presenceChannel, setPresenceChannel] = useState<any>(null);
  const searchParams = useSearchParams();
  const selectedClientIdFromQuery =
    searchParams?.get("selectedClientId") || null;
  ...
  const [adminView, setAdminView] = useState<"messages" | "campaigns">(
    "messages",
  );
    ...
    if (isAdmin) {
      ...
      const loadAdmin = async () => {
        try {
          ...
          if (cachedAdminId) {
            const { user: cachedAdmin, error: getUserError } =
              await userService.getUser(cachedAdminId);
  ...
  const handleSummarizeConversation = async () => {
    ...
    try {
      const conversationText = visibleMessages
        .map(
          (msg) =>
            `${msg.senderId === user.id ? "You" : isAdmin ? selectedClient?.name : "Admin"}: ${msg.text}`,
        )
        .join("\n");
  ...
  const visibleMessages = useMemo(() => {
    ...
    if (isAdmin) {
      ...
      internal = filteredMessages.filter(
        (m) =>
  ...
  // Scroll to bottom without hijacking users reading earlier messages.
  useEffect(() => {
    const nextCount = visibleMessages.length;
    const previousCount = previousMessageCountRef.current;
    const grew = nextCount > previousCount;
    const lastMessage = visibleMessages[nextCount - 1];
    const sentByCurrentUser =
      lastMessage?.senderId === user.id || lastMessage?.role === "model";

    if (selectedClient || selectedCRMContact) {
      if (
        previousCount === 0 ||
        sentByCurrentUser ||
        (grew && wasNearBottomRef.current)
      ) {
  ...
  }, [visibleMessages, selectedClient?.id, selectedCRMContact?.id, user.id]);
  ...
  const handleSend = () => {
    ...
    if (isAdmin) {
      ...
      if (!recipientId) {
        toast.error(
          "Connecting to admin... Please wait a moment and try again.",
        );
        return;
      }
    }
  ...
  const isRecipientTyping = isAdmin
    ? selectedClient
      ? typingUsers.has(selectedClient.id)
      : false
    : adminUser
      ? typingUsers.has(adminUser.id)
      : false;
  ...
  const messageStats = useMemo<ModuleStat[]>(() => {
    ...
    const priority = filteredMessages.filter(
    ...
    const channels = new Set(
      filteredMessages
        .map((m) => m.source)
        .filter((s): s is string => !!s && s !== "internal"),
    );
    return [
      {
              ...
              <div className="flex-1 flex flex-col z-10 bg-slate-900/20 min-w-0 overflow-hidden h-full">
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={async () => {
                        toast.loading("Nexus: Triaging support messages...", {
                          id: "nexus-support",
                        });
                  ...
                  {isAdmin && !isMobile && (
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <Bot
                          size={14}
                          className={autoReplyEnabled ? "animate-pulse" : ""}
                        />
                ...
                <div
                  ...
                  className="flex-1 overflow-y-auto p-3 sm:p-6 relative custom-scrollbar scroll-pb-28"
                  style={{ minHeight: 0 }}
                >
                    ...
                    <>
                      ...
                      <div className="space-y-1">
                        {visibleMessages.map((msg, index) => {
                          const isOwn =
                            msg.senderId === user.id || msg.role === "model";
                          ...
                          const channelBadge =
                            channelSource && channelSource !== "internal"
                              ? channelSource === "gmail" ||
                                channelSource === "zoho" ||
                                channelSource === "sendgrid" ||
                                channelSource === "resend" ||
                                channelSource === "brevo"
                                ? "✉️"
                  ...
                  <div className="p-3 md:p-5 border-t border-slate-700 relative bg-slate-900/40 backdrop-blur-md flex-shrink-0">
                    ...
                    <div className="flex gap-2 md:gap-3 relative items-end w-full">
                      <div className="relative flex items-end gap-1 md:gap-2 flex-shrink-0">
                        ...
                        >
                          <Flag
                            className={`w-4 h-4 sm:w-5 sm:h-5 ${priority !== "normal" ? "fill-current" : ""}`}
                          />
                        </button>
                        ...
                        {isAdmin && (
                          ...
                          >
                            <Wand2
                              className={`w-4 h-4 sm:w-5 sm:h-5 ${isAIGenerating ? "animate-pulse" : ""}`}
                            />
                          </button>
                        )}
