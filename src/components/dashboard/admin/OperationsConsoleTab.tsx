    ...
    <div className="space-y-8 animate-fade-in max-w-5xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-400 type-card-description">Business-readable error snapshot plus structured incident intake.</p>
        </div>
        <button
          type="button"
          onClick={() => void loadBrief()}
          disabled={loading}
          className="ml-auto inline-flex items-center gap-2 rounded-lg border border-slate-700 px-3 py-2 type-ui font-semibold text-slate-200 hover:bg-slate-800 disabled:opacity-50"
        >
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <pre className="whitespace-pre-wrap type-ui text-slate-300 font-sans leading-relaxed">{brief || (loading ? 'Loading…' : 'No data.')}</pre>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-xl border border-slate-800 bg-slate-900/60 p-6 space-y-4">
          ...
          <div>
            <label className="block type-label font-semibold text-slate-500 mb-1">Title</label>
            <input
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              className="w-full rounded-lg bg-slate-950 border border-slate-700 px-3 py-2 type-ui text-white"
              placeholder="Short headline"
            />
          </div>
          <div>
            <label className="block type-label font-semibold text-slate-500 mb-1">Product area</label>
            <input
              value={area}
          ...
          <div>
            <label className="block type-label font-semibold text-slate-500 mb-1">User impact</label>
            <textarea
              value={impact}
              onChange={(e) => setImpact(e.target.value)}
              className="w-full rounded-lg bg-slate-950 border border-slate-700 px-3 py-2 type-ui text-white min-h-[72px]"
              placeholder="Who is blocked and how severely?"
            />
          </div>
          <div>
            <label className="block type-label font-semibold text-slate-500 mb-1">Steps to reproduce</label>
            <textarea
              value={stepsToReproduce}
              onChange={(e) => setStepsToReproduce(e.target.value)}
              className="w-full rounded-lg bg-slate-950 border border-slate-700 px-3 py-2 type-ui text-white min-h-[88px]"
            />
          </div>
          <div>
            <label className="block type-label font-semibold text-slate-500 mb-1">Expected behavior</label>
            <textarea
            ...
            {recent.length === 0 ? (
              ...
              recent.map((row, idx) => (
                <div key={row.id || idx} className="rounded-lg border border-slate-800 bg-slate-950/50 p-3">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 type-ui text-slate-500">
