...
const CRMTab: React.FC<CRMTabProps> = ({ userId, userRole }) => {
    ...
    return (
        <>
            <div className="h-full flex flex-col p-2 lg:p-8 overflow-hidden custom-scrollbar space-y-2 lg:space-y-8 bg-[radial-gradient(circle_at_top_right,rgba(20,184,166,0.05),transparent_40%)]">
                {/* Elite Client Relations Header */}
                <div className={`flex flex-col lg:flex-row lg:items-end justify-between gap-4 lg:gap-8 pb-4 lg:pb-8 border-b border-white/5 relative ${selectedClient ? 'hidden lg:flex' : 'flex'}`}>
                    <div className="flex-1 relative z-10">
                        ...
                        >
                            <div className="p-2 lg:p-3 bg-teal-500 rounded-xl lg:rounded-2xl shadow-2xl shadow-teal-500/40 rotate-3">
                                <Briefcase className="w-5 h-5 lg:w-8 lg:h-8 text-slate-900" />
                            </div>
                            <div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 relative z-10">
                        <div className="relative group">
                            ...
                            <input
                                ...
                                className="w-full lg:w-72 bg-black/40 border border-white/5 rounded-xl lg:rounded-2xl pl-10 lg:pl-12 pr-4 lg:pr-6 py-2 lg:py-3 text-[10px] font-mono tracking-widest text-white focus:border-teal-500/40 outline-none transition-all placeholder:text-slate-700 shadow-inner"
                            />
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 lg:gap-2">
                            <motion.button
                                ...
                                className="p-2 lg:p-3 bg-slate-900/50 border border-white/5 rounded-xl lg:rounded-2xl text-slate-400 hover:text-teal-400 transition-all hover:bg-teal-500/5"
                                title="Import Leads"
                            >
                            ...
                            <motion.button
                                ...
                                className="bg-teal-500 hover:bg-teal-400 text-slate-950 rounded-xl lg:rounded-2xl h-9 lg:h-12 px-4 lg:px-8 shadow-[0_10px_30px_rgba(20,184,166,0.3)] transition-all flex items-center gap-1.5 lg:gap-3 group ml-1 lg:ml-2"
                            >
                                <Plus className="w-3.5 h-3.5 lg:w-4 lg:h-4 font-bold group-hover:rotate-90 transition-transform duration-500" />
                                <span className="font-black text-[10px] uppercase tracking-[0.2em] hidden sm:block">New Relation</span>
                                <span className="font-black text-[10px] uppercase tracking-[0.2em] sm:hidden">New</span>
                            </motion.button>
                        </div>
                    </div>
                </div>

                <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-2 lg:gap-6">
                    {/* Client Intelligence Directory */}
                    <div className={`lg:col-span-1 glass-panel rounded-2xl lg:rounded-[2rem] border border-white/5 overflow-hidden flex flex-col bg-slate-900/20 backdrop-blur-2xl shadow-2xl ${selectedClient ? 'hidden lg:flex' : 'flex'}`} style={{ minHeight: '500px' }}>
                        <div className="p-4 lg:p-6 border-b border-white/5 bg-slate-950/20 flex justify-between items-center">
                                    ...
                                    {clients.map((client, idx) => (
                                        ...
                                        >
                                            <motion.div
                                                ...
                                                className={`p-3 lg:p-4 rounded-xl lg:rounded-2xl border transition-all cursor-pointer group flex items-center justify-between relative overflow-hidden ${selectedClient?.id === client.id
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-3 lg:gap-4 overflow-hidden relative z-10">
                                                    <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-lg lg:rounded-xl flex items-center justify-center text-lg lg:text-xl font-black flex-shrink-0 transition-all duration-500 ${selectedClient?.id === client.id ? 'bg-teal-500 text-slate-950 scale-110 rotate-3' : 'bg-slate-800 text-slate-400 group-hover:bg-slate-700'
                                                        }`}>
                    ...
                    <div className={`lg:col-span-2 glass-panel rounded-2xl lg:rounded-[2rem] border border-white/5 overflow-hidden flex flex-col relative bg-slate-900/40 backdrop-blur-2xl shadow-2xl ${!selectedClient ? 'hidden lg:flex' : 'flex focus-in'}`}>
                        <AnimatePresence mode="wait">
                            {selectedClient ? (
                                ...
                                >
                                    {/* Command Banner */}
                                    <div className="h-32 md:h-64 bg-[url('https://images.unsplash.com/photo-1557683316-973673baf926?q=80&w=2070&auto=format&fit=crop')] bg-cover bg-center relative shrink-0">
                                        ...
                                        <button
                                            ...
                                            className="lg:hidden absolute top-4 left-4 p-2 bg-black/40 backdrop-blur-md rounded-full text-white hover:bg-white/20 transition-all z-20 shadow-xl border border-white/10"
                                        >
                                            <X className="w-4 h-4" />
                                        </button>

                                        <div className="absolute -bottom-8 md:-bottom-14 left-4 md:left-14 flex items-end gap-3 md:gap-10">
                                            <motion.div
                                                layoutId={`avatar-${selectedClient.id}`}
                                                className="w-20 h-20 md:w-36 md:h-36 rounded-2xl md:rounded-[2.5rem] bg-slate-900 border-2 md:border-4 border-slate-950 flex items-center justify-center text-3xl md:text-6xl font-black text-white shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden group shrink-0"
                                            >
                                            ...
                                            <div className="mb-2 md:mb-6 pb-1 md:pb-2">
                                                <motion.h1
                                                    initial={{ opacity: 0, y: 15 }}
                                                    animate={{ opacity: 1, y: 0 }}
                                                    transition={{ delay: 0.2 }}
                                                    className="text-2xl md:text-5xl font-black text-white tracking-tighter uppercase leading-[0.8] pr-4 max-w-[200px] md:max-w-none truncate md:overflow-visible md:whitespace-normal"
                                                >
                                                    {selectedClient.name}
                                                </motion.h1>
                                                <div className="flex flex-wrap items-center gap-2 md:gap-4 mt-2 md:mt-4">
                                                    <span className={`px-2 md:px-3.5 py-1 md:py-1.5 rounded-lg md:rounded-xl text-[8px] md:text-[10px] font-black uppercase tracking-[0.25em] border ${selectedClient.salesStage === 'customer' ? 'bg-green-500/10 text-green-400 border-green-500/30' : 'bg-teal-500/10 text-teal-400 border-teal-500/30'
                                                        }`}>
                                                        {selectedClient.salesStage}
                                                    </span>
                                                    <div className="hidden sm:flex items-center gap-1.5 md:gap-2 text-[8px] md:text-[10px] font-mono text-slate-500 uppercase tracking-widest bg-white/5 px-2 md:px-3 py-1 md:py-1.5 rounded-lg md:rounded-xl border border-white/5">
                                                        <Briefcase className="w-3 h-3 md:w-3.5 md:h-3.5 text-teal-500" />
                                                        {selectedClient.industry || 'Classified'}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

                                        <div className="hidden md:flex absolute top-8 right-8 gap-3">
                                            <motion.button
                                                whileHover={{ scale: 1.05 }}
                                                whileTap={{ scale: 0.95 }}
                                    ...
                                    <div className="md:hidden flex flex-wrap gap-2 px-4 mt-12 items-center z-10 relative">
                                        <motion.button
                                            whileHover={{ scale: 1.05 }}
                                            whileTap={{ scale: 0.95 }}
                                            onClick={() => setShowProjectModal(true)}
                                            className="flex-1 bg-teal-500 hover:bg-teal-400 text-slate-950 rounded-xl py-3 justify-center transition-all flex items-center gap-2 font-black text-[10px] uppercase tracking-[0.2em] shadow-lg shadow-teal-500/20"
                                        >
                                        ...
                                        <motion.button
                                            ...
                                            whileTap={{ scale: 0.95 }}
                                            onClick={() => handleEditClient(selectedClient)}
                                            className="p-3 bg-white/5 border border-white/10 rounded-xl text-slate-400 hover:text-white transition-all w-12 flex justify-center items-center"
                                        >
                                        ...
                                        <motion.button
                                            ...
                                            whileTap={{ scale: 0.95 }}
                                            onClick={() => handleDeleteClient(selectedClient.id)}
                                            className="p-3 bg-red-500/5 border border-red-500/20 rounded-xl text-red-500 hover:text-red-400 transition-all w-12 flex justify-center items-center"
                                        >
                                            <Trash2 className="w-4 h-4" />
                                        </motion.button>
                                    </div>

                                    <div className="mt-6 md:mt-24 px-4 md:px-16 pb-8 md:pb-16 space-y-6 md:space-y-16">
                                        ...
                                        <motion.div
                                            ...
                                            className="p-5 md:p-10 rounded-2xl md:rounded-[3.5rem] bg-slate-950/30 border border-white/5 relative overflow-hidden shadow-inner group/log mt-4 md:mt-0"
                                        >
                                            <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-teal-500/20 to-transparent" />
                                            <div className="text-[10px] md:text-[12px] text-slate-500 uppercase font-black tracking-[0.4em] mb-4 md:mb-8 flex items-center gap-3 md:gap-4">
                                                <FileText className="w-4 h-4 md:w-5 md:h-5 text-teal-500/50 group-hover/log:scale-110 transition-transform" />
                                                Strategic Intelligence Narrative
                                            </div>
                                            <p className="text-slate-400 text-sm md:text-lg leading-[1.6] md:leading-[1.8] font-medium italic md:indent-8">
