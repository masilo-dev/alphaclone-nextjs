  ...
  <main className="min-h-screen page-network-bg marketing-theme bg-transparent text-white">
    <section className="relative overflow-hidden border-b border-slate-800/50 bg-gradient-to-br from-slate-900 via-slate-950 to-[#071f2a] py-16 sm:py-24">
      ...
      <div className="mx-auto grid max-w-5xl gap-12 px-4 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
        <div>
          <p className="inline-flex items-center rounded-full border border-teal-400/25 bg-teal-400/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.14em] text-teal-300">A practical guide for first-time teams</p>
          <h1 className="mt-5 max-w-3xl text-4xl font-bold leading-[1.08] tracking-tight sm:text-6xl">Start with one useful thing for your business.</h1>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Link href="/auth/login?register=true" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-teal-500 px-5 py-3 text-sm font-semibold text-slate-950 transition-colors hover:bg-teal-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-200">Create an account <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link>
            <a href="#first-wins" className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-600 px-5 py-3 text-sm font-semibold text-white transition-colors hover:border-teal-300 hover:bg-teal-500/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-teal-200">See what you can do first</a>
          </div>
          <p className="mt-5 text-xs text-slate-500">Choose a direction now. Change it later without losing access to the rest of your workspace.</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-5 shadow-2xl shadow-black/20 backdrop-blur-sm sm:p-6">
          <p className="text-xs font-semibold uppercase tracking-[0.12em] text-teal-300">Your first session</p>
          <div className="mt-5 space-y-4">
            {STARTING_STEPS.slice(0, 3).map((step, index) => (
              <div key={step.number} className="flex gap-3">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-teal-400/15 text-xs font-bold text-teal-200">{step.number}</span>
                <div><p className="text-sm font-semibold text-white">{step.title}</p><p className="mt-1 text-xs leading-relaxed text-slate-400">{index === 0 ? 'Confirm your email and enter your workspace.' : index === 1 ? 'Tell AlphaClone what matters most right now.' : 'Complete one small action and see the result.'}</p></div>
              </div>
            ))}
          </div>
          <div className="mt-5 border-t border-white/10 pt-4 text-xs text-slate-400"><span className="font-semibold text-teal-300">Always in control:</span> review messages, invoices, and posts before they leave AlphaClone.</div>
        </div>
      </div>
    </section>
