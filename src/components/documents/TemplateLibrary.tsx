...
export function TemplateLibrary() {
  ...
  return (
    <div className="space-y-6">
      <section>
        <h3 className="type-ui font-semibold text-[var(--ws-text-primary)] mb-3 flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {themes.map((theme) => (
            ...
            >
              <p className="type-card-description font-medium text-[var(--ws-text-primary)]">{theme.name}</p>
      ...
      <section>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {INDUSTRY_TEMPLATES.map((tpl) => (
            ...
            >
              <div>
                <p className="type-card-description font-medium text-[var(--ws-text-primary)]">{tpl.name}</p>
                <p className="type-card-description text-[var(--ws-text-tertiary)] capitalize">{tpl.industry} · {tpl.type}</p>
              </div>
              <Link
                href={`/dashboard/business/documents?template=${encodeURIComponent(tpl.name)}`}
                className="ac-workspace-action-btn ac-workspace-action-btn--secondary type-ui min-h-8 px-2.5 flex items-center gap-1"
              >
