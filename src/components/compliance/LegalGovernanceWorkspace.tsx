    ...
    <main className="min-h-screen bg-slate-950 px-4 py-8 text-slate-100">
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 flex items-start gap-3">
          ...
          <div><p className="type-card-description text-teal-300">Settings</p><h1 className="text-3xl font-semibold">{copy.title}</h1><p className="mt-2 max-w-3xl text-slate-400">{copy.description}</p></div>
        </div>
        <div className="grid gap-6 lg:grid-cols-[240px_1fr]">
          <nav aria-label="Legal settings" className="rounded-2xl border border-white/10 bg-slate-900 p-2">
            {sections.map(([label, href]) => <Link key={href} href={href} className={`block rounded-xl px-3 py-2.5 type-ui ${pathname === href ? 'bg-teal-400/10 text-teal-300' : 'text-slate-300 hover:bg-white/5'}`}>{label}</Link>)}
          </nav>
          <section className="space-y-5">
            <div className="grid gap-3 sm:grid-cols-3">
              {[['Published versions', 'Versioned and immutable', BookOpen], ['Review gate', 'Approval required', CheckCircle2], ['Localisation', 'Approved fallbacks', Globe2]].map(([label, detail, Icon]) => (
                <div key={String(label)} className="rounded-2xl border border-white/10 bg-slate-900 p-4"><Icon aria-hidden className="mb-3 h-5 w-5 text-teal-300" /><p className="font-medium">{String(label)}</p><p className="type-card-description text-slate-400">{String(detail)}</p></div>
              ))}
            </div>
            <div className="rounded-2xl border border-white/10 bg-slate-900">
              <div className="flex items-center justify-between border-b border-white/10 p-5"><div><h2 className="font-semibold">Governed records</h2><p className="type-ui text-slate-400">Content is tenant-isolated and retains full version lineage.</p></div><button type="button" className="rounded-xl bg-teal-400 px-4 py-2 type-ui font-semibold text-slate-950">Create draft</button></div>
              <div className="p-8 text-center"><Clock3 aria-hidden className="mx-auto mb-3 h-8 w-8 text-slate-500" /><p className="font-medium">No records loaded</p><p className="mt-1 type-card-description text-slate-400">Connect this workspace to a tenant to view and edit governed records.</p></div>
            </div>
            <div className="rounded-xl border border-amber-300/20 bg-amber-300/5 p-4 type-ui text-amber-100">Creating a policy does not establish legal compliance. Obtain qualified legal review before publication.</div>
          </section>
        </div>
      </div>
    </main>
