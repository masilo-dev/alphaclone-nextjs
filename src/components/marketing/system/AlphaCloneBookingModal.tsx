...
export default function AlphaCloneBookingModal() {
  ...
  // Initialise Calendly widget when script loaded & modal opens
  useEffect(() => {
    if (!isOpen || !validUrl) return;
    ...
    const initWidget = () => {
      ...
      if (win.Calendly && typeof win.Calendly.initInlineWidget === 'function') {
        ...
        // Fallback to iframe if Calendly SDK object is not available
        setWidgetReady(true);
      }
    };
    ...
    // Timeout safety fallback if Calendly script fails to report ready within 6s
