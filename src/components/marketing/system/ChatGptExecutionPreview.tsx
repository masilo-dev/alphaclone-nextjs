      ...
      <div className={`space-y-4 bg-[#212121] ${compact ? 'p-4 max-h-[320px] overflow-y-auto' : 'p-5 sm:p-6'}`}>
        {MESSAGES.map((msg, index) => (
          ...
          >
              ...
              {msg.tools && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {msg.tools.map((tool) => (
                    <span
                      key={tool}
                      className="inline-flex items-center gap-1 rounded-md border border-cyan-400/25 bg-cyan-400/10 px-2 py-0.5 font-mono type-ui text-cyan-200"
                    >
            ...
            {msg.role === 'user' && (
              <div className="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-full bg-slate-600 type-ui font-bold text-white">
                You
              </div>
            )}
          </div>
