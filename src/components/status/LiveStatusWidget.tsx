    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border border-white/10 bg-slate-900/60 p-4 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <span className="type-caption font-semibold text-slate-300">
            Live Monitoring Active • Auto-refreshes in <span className="font-mono font-bold text-teal-400">{countdown}s</span>
          </span>
          <button
            onClick={fetchLiveStatus}
            disabled={isRefreshing}
            className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-slate-800 px-2.5 py-1 type-caption font-medium text-slate-300 hover:bg-slate-700 hover:text-white transition-all disabled:opacity-50"
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-2 rounded-lg bg-teal-500/10 border border-teal-500/30 px-3.5 py-1.5 type-caption font-bold text-teal-300 hover:bg-teal-500/20 transition-all shadow-sm"
          >
      ...
      <div className="rounded-2xl border border-white/10 bg-slate-950/90 p-6 shadow-xl backdrop-blur-md">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="type-card-description text-slate-400 mt-0.5">Historical daily platform uptime posture (Past 90 Days)</p>
          </div>
          <span className="type-caption font-mono font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-full">
            100% Uptime
          </span>
        </div>
        ...
        <div className="flex items-center gap-1 overflow-x-auto py-2 scrollbar-thin">
          {Array.from({ length: 90 }).map((_, i) => (
            ...
            >
              <div className="absolute bottom-12 left-1/2 -translate-x-1/2 hidden group-hover:block z-20 whitespace-nowrap rounded-md border border-slate-700 bg-slate-900 px-2.5 py-1 type-ui font-medium text-slate-200 shadow-xl">
        ...
        <div className="mt-3 flex items-center justify-between type-ui text-slate-500 font-medium">
          ...
          <div className="w-full max-w-md rounded-2xl border border-white/15 bg-slate-950 p-6 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                ...
                className="text-slate-400 hover:text-white type-ui font-bold"
              >
                ✕
              </button>
            </div>
            ...
            {subscribed ? (
              <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-5 text-center">
                ...
                <h4 className="type-ui font-bold text-emerald-200">Subscribed Successfully!</h4>
                <p className="type-card-description text-slate-300 mt-1">
                  You will receive real-time incident notifications at <span className="font-semibold text-white">{email}</span>.
                </p>
              </div>
            ) : (
              <form onSubmit={handleSubscribe} className="space-y-4">
                <p className="type-card-description leading-5 text-slate-300">
                  Get instant notification emails whenever a platform engine experiences a status change or scheduled maintenance.
                </p>
                <div>
                  <label className="block type-label font-semibold text-slate-400 mb-1.5">Notification Email</label>
                  <input
                    type="email"
                    required
                    ...
                    className="w-full rounded-xl border border-white/10 bg-slate-900 px-4 py-2.5 type-ui text-white placeholder-slate-500 focus:border-teal-500 focus:outline-none"
                  />
                </div>
                <div className="flex justify-end gap-2 pt-2">
                  <button
                    type="button"
                    onClick={() => setSubscribeOpen(false)}
                    className="rounded-xl border border-white/10 px-4 py-2 type-caption font-semibold text-slate-300 hover:bg-slate-900"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="rounded-xl bg-gradient-to-r from-teal-500 to-cyan-500 px-5 py-2 type-caption font-bold text-slate-950 hover:from-teal-400 hover:to-cyan-400 transition-all shadow-md"
                  >
