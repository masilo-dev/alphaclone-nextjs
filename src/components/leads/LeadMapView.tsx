...
export default function LeadMapView({ leads, center = [40.7128, -74.006], zoom = 11 }: LeadMapViewProps) {
  ...
  return (
    <div className="relative w-full rounded-xl overflow-hidden border border-slate-700 shadow-2xl" style={{ height: '480px' }}>
      {/* Legend */}
      <div className="absolute top-3 right-3 z-[1000] flex flex-col gap-1 bg-slate-900/90 backdrop-blur-md rounded-lg px-3 py-2 border border-slate-700 text-[10px] font-semibold">
      ...
      {/* Pin count */}
      <div className="absolute top-3 left-3 z-[1000] bg-slate-900/90 backdrop-blur-md rounded-lg px-3 py-1.5 border border-slate-700 text-[11px] font-bold text-white">
        📍 {pinnable.length} <span className="text-slate-400 font-normal">of {leads.length} leads mapped</span>
      </div>
      ...
      >
        ...
        {pinnable.map((lead, idx) => (
          ...
          >
            <Popup maxWidth={260} className="lead-popup">
              <div className="p-1 space-y-1.5" style={{ fontFamily: 'system-ui, sans-serif' }}>
                ...
                {lead.rating && (
                  <p className="text-[11px] font-semibold text-amber-600">
                    {'★'.repeat(Math.round(lead.rating))}{'☆'.repeat(5 - Math.round(lead.rating))} {lead.rating.toFixed(1)}
                  </p>
                )}

                {/* Address */}
                {lead.address && (
                  <p className="text-[10px] text-slate-600 leading-snug">📍 {lead.address}</p>
                )}

                {/* Phone */}
                {lead.phone && (
                  <p className="text-[10px] text-slate-700">📞 {lead.phone}</p>
                )}
