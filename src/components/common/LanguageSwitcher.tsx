    ...
    return (
        <div className="relative">
            <button
                ...
                className="flex items-center gap-2 px-2.5 py-2 rounded-lg bg-[var(--surface-secondary)] hover:bg-[var(--surface-hover)] border border-[var(--border-default)] text-[var(--text-secondary)] text-xs transition-colors"
            >
                <Globe className="w-4 h-4 text-[var(--text-muted)]" aria-hidden="true" />
                <span className="font-semibold">{current.nativeName}</span>
                <span className="rounded bg-[var(--surface-tertiary)] px-1.5 py-0.5 text-[10px] font-bold tracking-wide text-[var(--text-muted)]">{languageCode}</span>
            ...
            {isOpen && (
                <>
                    ...
                    <div className="absolute right-0 mt-2 w-56 bg-[var(--surface-elevated)] border border-[var(--border-default)] rounded-xl shadow-2xl z-50 overflow-hidden" role="listbox" aria-label={t('Language')}>
                        <div className="px-3 py-2 border-b border-[var(--border-default)] text-[10px] font-semibold uppercase tracking-wider text-[var(--text-muted)]">
                            {t('Choose language')}
                        </div>
                        {LANGUAGES.map((lang) => (
                            <button
                            ...
                            >
                                <div className="text-left">
                                    ...
                                    <span className="block text-xs text-[var(--text-muted)]">{lang.label} · {lang.code.toUpperCase()}</span>
                                </div>
                                {language === lang.code && (
