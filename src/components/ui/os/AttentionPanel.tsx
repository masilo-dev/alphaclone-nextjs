...
export function AttentionPanel({
  ...
  return (
    <section className={cn(WORKSPACE.panel.base, 'p-4 md:p-5', className)}>
      ...
      {items.length === 0 ? (
        <p className="mt-4 text-sm text-[var(--ws-text-secondary)]">{emptyMessage}</p>
      ) : (
            ...
            <li key={item.id}>
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-[var(--ws-text-primary)]">{item.reason}</p>
                  <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-[var(--ws-text-muted)]">
                    {item.record ? <span>{item.record}</span> : null}
