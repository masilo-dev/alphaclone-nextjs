/**
 * Route contract lifecycle through Bonnie durable runtime when enabled.
 */

import { isDurableRuntimeEnabled } from '@/lib/bonnie/runtime/types';
import { enqueueContractLifecycleTask } from '@/lib/contracts/contractLifecycleDurableTask';
import { start } from 'workflow/api';
import { contractLifecycleWorkflow } from '@/workflows/contract-lifecycle';

export async function queueContractLifecycle(input: {
  tenantId: string;
  contractId: string;
  userId?: string;
}): Promise<{ durable: boolean; run_id: string; task_id?: string; poll_tool: string }> {
  if (isDurableRuntimeEnabled()) {
    const enqueued = await enqueueContractLifecycleTask({
      tenantId: input.tenantId,
      userId: input.userId,
      contractId: input.contractId,
    });
    return {
      durable: true,
      run_id: enqueued.runId,
      task_id: enqueued.taskId,
      poll_tool: 'get_outcome_status',
    };
  }

  const { runId } = await start(contractLifecycleWorkflow, [
    { contractId: input.contractId, tenantId: input.tenantId },
  ]);
  return {
    durable: false,
    run_id: runId,
    poll_tool: 'get_outcome_status',
  };
}
