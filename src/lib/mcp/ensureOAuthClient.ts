 ...
 */
export function getToolCatalogModeForClient(
...
function mergeRedirectUris(existing: unknown, seed: string[]): string[] {
  const fromDb = Array.isArray(existing)
    ? existing.map((u) => String(u || "").trim()).filter(Boolean)
    : [];
  return [...new Set([...fromDb, ...seed])];
}

/** Upsert a known platform MCP OAuth client (idempotent). Merges redirect URIs — never shrinks. */
export async function ensurePlatformMcpOAuthClient(
  ...
  try {
    ...
    if (!existing.error && existing.data) {
      existingUris = existing.data.redirect_uris;
    }
  } catch {
    // proceed with seed-only
  }

  const redirect_uris = mergeRedirectUris(existingUris, seed.redirect_uris);
  ...
  let { error } = await supabase
    .from("mcp_oauth_clients")
    .upsert(fullRow, { onConflict: "client_id" });

  if (isMissingColumnError(error)) {
    const {
      grant_types: _g,
      response_types: _r,
      token_endpoint_auth_method: _t,
      is_active: _a,
      metadata: _m,
      ...minimal
    } = fullRow;
  ...
  if (error) {
    console.warn(
      "[MCP OAuth] Failed to ensure platform client:",
      clientId,
      error.message,
    );
    return false;
