...
registerTool('social-publishing', {
  ...
  description:
    'Generate an image with the configured AI provider, upload it permanently, then create or publish a social post. If image generation fails (billing inactive, rate limit, provider outage), AlphaClone returns the real provider error — it does NOT publish unless fallback_to_text_only=true and the user/agent explicitly chose caption-only.',
  inputSchema: z.object({
  ...
  handler: async (args, ctx) => {
    ...
    if (!imageResult.ok && !args.fallback_to_text_only) {
      return toMcpContent(
        errorResult(
          ...
          imageResult.error.message,
          {
            image_generation: {
              ...
              provider: imageResult.error.provider || null,
            },
            post_not_created: true,
            next_steps: [
              'Tell the user the AI image provider failed (this is not a Facebook or LinkedIn error).',
              'Ask whether to publish caption-only, retry with another provider/image, or supply upload_social_media bytes.',
              'Caption-only: call create_social_post or publish_social_post with the same caption and no media.',
              'Or call this tool again with fallback_to_text_only=true after explicit user approval.',
            ],
          }
        )
      );
    ...
    let imageGeneration:
      | { success: true; provider: string; revised_prompt?: string | null }
      | { success: false; code: string; message: string; provider?: string };

    if (imageResult.ok) {
      const asset = await uploadSocialMedia({
        tenantId,
        userId,
        filename: `bonnie-${crypto.randomUUID()}.png`,
        mimeType: 'image/png',
        contentBase64: imageResult.base64,
        altText: args.alt_text,
      });
      mediaAssetId = asset.media_asset_id;
      mediaUrl = buildPublicMediaUrl(asset.media_asset_id);
      imageGeneration = {
        success: true,
      ...
      imageGeneration = {
        ...
        code: imageResult.error.code,
        message: imageResult.error.message,
        provider: imageResult.error.provider,
      };
    }

    const { executeTool } = await import('../tool-registry');
    const post = await executeTool(tenantId, userId, 'create_social_post', {
      tenant_id: tenantId,
    ...
    return {
      generated: imageGeneration.success,
      fallback: imageGeneration.success ? null : 'text_only',
      fallback_approved: Boolean(args.fallback_to_text_only && !imageGeneration.success),
      image_generation: imageGeneration,
      warning: imageGeneration.success
        ? null
        : `${imageGeneration.message} Published caption-only because fallback_to_text_only=true.`,
      media_asset_id: mediaAssetId || null,
      media_url: mediaUrl,
      post,
      verification: {
        media_ready: imageGeneration.success,
        post_action_returned: Boolean(post),
        text_only_fallback: !imageGeneration.success,
      },
    };
  },
});
