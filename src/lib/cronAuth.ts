...
/**
 * Railway Cron sets `x-railway-cron: 1`. Other schedulers should send `Authorization: Bearer ${CRON_SECRET}`.
...
export function denyIfCronUnauthorized(req: NextRequest): NextResponse | null {
    ...
    if (process.env.NODE_ENV === 'production') {
        return NextResponse.json(
            { error: 'Cron misconfigured: set CRON_SECRET/INTERNAL_API_KEY or invoke from Railway Cron' },
            { status: 503 }
        );
    }
