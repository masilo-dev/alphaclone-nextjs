...
export async function isEmailSuppressed(
  ...
  if (legacy.error || lifecycle.error) {
    console.error(
      "[email/suppression] lookup failed:",
      legacy.error || lifecycle.error,
    );
    return true;
  }
  return Boolean(legacy.data?.id || lifecycle.data?.id);
}
    ...
    .upsert(
      {
        tenant_id: args.tenantId,
        channel: "email",
        normalized_recipient: normalizedEmail,
        reason: args.reason,
        source: args.provider || "email_suppression",
      },
      { onConflict: "tenant_id,channel,normalized_recipient" },
    );
  if (lifecycleError)
    console.error(
      "[email/suppression] lifecycle upsert failed:",
      lifecycleError,
    );
}
...
export async function syncSuppressionCleanup(args: {
  ...
  const normalizedStatus =
    args.reason === "unsubscribe" ? "unsubscribed" : "bounced";
  ...
  if (contactResult.error) {
    console.error(
      "[email/suppression] contact cleanup failed:",
      contactResult.error,
    );
  }
  if (outreachResult.error) {
    console.error(
      "[email/suppression] outreach cleanup failed:",
