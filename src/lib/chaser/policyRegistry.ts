...
const DEFAULT_POLICIES: Record<ChasePolicyKey, ChasePolicyDefinition> = {
  ...
  booking_chaser: {
    key: 'booking_chaser',
    label: 'Booking chaser',
    entityType: 'booking',
    detectDescription: 'Upcoming booking without confirmation or cancelled reminder still running',
    initialAction: 'Confirm attendance internally',
    escalationAction: 'Owner alert',
    verifiedStopOutcomes: ['completed', 'cancelled', 'no_show'],
    defaultIntervalHours: [24, 24],
    maxAttempts: 3,
    escalationAfterAttempts: 2,
    channel: 'notification',
    defaultAutomationMode: 'internal',
  ...
  document_chaser: {
    key: 'document_chaser',
    label: 'Document chaser',
    entityType: 'document',
    detectDescription: 'Shared document not viewed or expiring',
    initialAction: 'Internal reminder',
    escalationAction: 'Owner alert',
    verifiedStopOutcomes: ['viewed', 'approved', 'expired', 'revoked'],
    defaultIntervalHours: [48, 120],
    maxAttempts: 3,
    escalationAfterAttempts: 2,
    channel: 'notification',
    defaultAutomationMode: 'internal',
  ...
  integration_chaser: {
    key: 'integration_chaser',
    label: 'Integration chaser',
    entityType: 'integration',
    detectDescription: 'Disconnected or failing integration',
    initialAction: 'Owner reconnect alert',
    escalationAction: 'System health escalation',
    verifiedStopOutcomes: ['connected', 'reauthorized', 'disabled'],
    defaultIntervalHours: [6, 24, 72],
    maxAttempts: 4,
    escalationAfterAttempts: 2,
    channel: 'notification',
    defaultAutomationMode: 'internal',
