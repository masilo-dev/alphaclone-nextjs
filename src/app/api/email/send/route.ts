...
export async function POST(req: NextRequest) {
  try {
    ...
    const { tenantId, to, subject, contactId, clientId, threadId, provider, document_file_ids, skipRecipientGate } = parsed.data;
    ...
    if (!result.success) {
      return NextResponse.json({ error: result.error || 'Failed to send email', code: result.code || 'SEND_FAILED', tried: result.errorDetails },
        { status: result.code === 'CONFIG_MISSING' || result.code === 'VALIDATION_ERROR' || result.code === 'EMAIL_PROVIDER_ACCOUNT_TENANT_MISMATCH' ? 400 : 503 });
    }
