    ...
    <main className="min-h-screen bg-slate-100 text-slate-900">
      <div className="mx-auto max-w-7xl px-4 py-8">
        <header className="mb-6">
          ...
          <p className="mt-1 type-card-description text-slate-600">
        ...
        <div className="mb-4 flex flex-wrap gap-2">
          {MODES.map((item) => (
            <button
              ...
              className={`rounded-md px-3 py-1.5 type-ui ${mode === item.id ? 'bg-slate-900 text-white' : 'bg-white text-slate-700 ring-1 ring-slate-200'}`}
            >
              {item.label}
            </button>
          ))}
        ...
        {error && (
          <div className="mb-4 rounded-md border border-red-200 bg-red-50 px-4 py-3 type-ui text-red-700">
            {error}
          </div>
        )}
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
              ...
              {(data?.samples || []).map((sample) => (
                <li key={sample.id}>
                  <button
                    ...
                    className={`w-full rounded-md px-3 py-2 text-left type-ui ${selectedId === sample.id ? 'bg-slate-900 text-white' : 'hover:bg-slate-50'}`}
                  >
                    <div className="font-medium">{sample.label}</div>
                    <div className={`type-caption ${selectedId === sample.id ? 'text-slate-200' : 'text-slate-500'}`}>
                      {sample.description}
                    </div>
                  </button>
                </li>
          ...
          <section className="rounded-lg bg-white p-4 shadow-sm ring-1 ring-slate-200">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="type-caption text-slate-500">{frameWidth}px frame</span>
            </div>
            <div className="overflow-auto rounded-md border border-slate-200 bg-slate-50 p-3">
              <div style={{ width: frameWidth, margin: '0 auto' }}>
                {selected?.html ? (
                  ...
                  <div className="py-16 text-center type-ui text-slate-500">Loading preview…</div>
                )}
              </div>
            </div>
            {selected?.text && (
              <details className="mt-4">
                <summary className="cursor-pointer type-ui font-medium text-slate-700">Plain-text version</summary>
                <pre className="mt-2 overflow-auto rounded-md bg-slate-50 p-3 type-caption text-slate-700">{selected.text}</pre>
              </details>
            )}
          </section>
        </div>
