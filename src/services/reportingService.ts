...
export const reportingService = {
    ...
    async exportToExcel(reportData: ReportData): Promise<{ blob: Blob | null; error: string | null }> {
        try {
            ...
            summary.addRow(['Generated', reportData.metadata.generatedAt]);
            summary.addRow(['Date Range', reportData.metadata.dateRange]);
            summary.addRow(['Generated By', reportData.metadata.generatedBy]);
            summary.addRow([]);
            summary.addRow(['Metric', 'Value']);
            summary.getRow(5).font = { bold: true };
            summary.addRow(['Total Projects', reportData.summary.totalProjects]);
            summary.addRow(['Total Revenue', reportData.summary.totalRevenue]);
            summary.addRow(['Total Clients', reportData.summary.totalClients]);
            summary.addRow(['Avg Project Duration', reportData.summary.averageProjectDuration]);

            for (const table of reportData.tables) {
                const sheetName = String(table.title || 'Data').slice(0, 31);
            ...
            return { blob: null, error: 'Excel export failed' };
        }
    },
