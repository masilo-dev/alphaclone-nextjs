...
export const DEFAULT_BUSINESS_AI_STATE: BusinessAIState = {
  ...
  audit: {
    ...
    human_review_actions: process.env.MCP_AUTO_EXECUTE === 'true' ? [] : ['send_invoice', 'send_transactional_email', 'send_whatsapp_message', 'update_contract_status'],
  },
  compliance: {
    dpa_ok: true,
    retention_ok: true,
...
function normalizeAudit(value: Partial<BusinessAIAuditState> | undefined): BusinessAIAuditState {
  const input = value || {};
  const defaultActions = process.env.MCP_AUTO_EXECUTE === 'true' ? [] : [...DEFAULT_BUSINESS_AI_STATE.audit.human_review_actions];
  return {
