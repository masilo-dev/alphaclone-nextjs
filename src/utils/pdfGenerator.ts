...
import { extractTenantBranding, type TenantBranding } from '@/lib/tenantBranding';
...
export const generateQuotePDF = (quote: Quote, items: QuoteItem[], tenant: Tenant) => {
    ...
    const accentColor = branding.secondaryColor || '#14b8a6';
    const companyName = branding.name || 'Unconfigured Business';
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(255, 255, 255);
    drawWrappedText(doc, companyName, 20, 22, 100, { fontSize: 16, maxLines: 2 });

    const startY = 42;
    const addressLines = (branding.businessAddress || tenant?.business_address || '').split('\n').filter(Boolean);
    let currentY = startY;
    ...
    const taxId = branding.taxId || branding.taxNumber || tenant?.tax_id;
    if (taxId) {
        doc.text(`Tax ID: ${taxId}`, 20, currentY + 4);
        currentY += 4;
    }
    ...
    // --- Footer ---
    addFooter(doc, pageWidth, pageHeight, branding);
    return doc;
};
...
export const generatePnLPDF = (
    ...
    doc.text(branding.legalName || branding.name || 'Financial Statement', 20, 20);
...
/**
 * Common Tenant-Aware PDF Footer
 */
const addFooter = (doc: jsPDF, pageWidth: number, pageHeight: number, brandingOrFooter?: TenantBranding | string) => {
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(148, 163, 184);

    let footerLeft = '';
    if (typeof brandingOrFooter === 'string') {
        footerLeft = brandingOrFooter;
    } else if (brandingOrFooter) {
        footerLeft = brandingOrFooter.documentFooterText ||
            (brandingOrFooter.legalName && brandingOrFooter.legalName !== brandingOrFooter.name
                ? `${brandingOrFooter.name} · ${brandingOrFooter.legalName}`
                : brandingOrFooter.name || '');
    }
    ...
    for (let i = 1; i <= pageCount; i++) {
        doc.setPage(i);
        if (footerLeft) {
            doc.text(footerLeft, 20, pageHeight - 10, { align: 'left' });
        }
